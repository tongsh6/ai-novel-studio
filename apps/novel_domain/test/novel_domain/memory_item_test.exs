defmodule NovelDomain.MemoryItemTest do
  ...
  describe "new/6" do
    test "creates a memory item with defaults" do
      item = MemoryItem.new("mem_001", "work_001", "主角叫张三", MemoryType.character_profile(), MemoryScope.work(), MemorySourceType.author_created())
  ...
  describe "confirm/1" do
    test "marks status as CONFIRMED" do
      item = MemoryItem.new("m1", "w1", "内容", MemoryType.world_rule(), MemoryScope.global(), MemorySourceType.author_confirmed())
  ...
  describe "iron_law?/1" do
    test "returns true for weight>=0.9 + CONFIRMED + locked + AUTHOR_CONFIRMED" do
      item =
        MemoryItem.new("m1", "w1", "内容", MemoryType.world_rule(), MemoryScope.global(), MemorySourceType.author_confirmed())
        |> MemoryItem.confirm()
        |> MemoryItem.lock()
