defmodule NovelApplication.AgentRunFlows.JudgmentPlan do
  @moduledoc """
  CP4（ADR-0025 决策 2 计划按需）：判断①判"复杂"时的跨能力真计划执行 flow。

  计划由模型基于能力目录制定（AgenticPlanDraftPlanner，plan_drafted 照发——真计划
  值得展示）；app 只做机械 cursor 推进与每步 Orchestrator 门禁。步 target 为能力
  目录成员（context_assemble / character_roster / character_design /
  character_evolution / plot_outline / world_building / prose_writing），多产物
  逐个进待采纳区停 S1/S2。计划耗尽而目标未达先停等作者（真计划修订是 CP4b）。
  """
  ...
  def steps(_spec) do
    raise ArgumentError, "judgment_plan_v1 requires next_step_planner/1"
  end
  ...
  # CP4：计划由模型基于能力目录制定（真计划，plan_drafted 照发）；app 不预制轨道。
  ...
  defp mechanical_await_author_decision(run, sequence) do
    AgentNextStepDecision.new(%{
      ...
      summary: "计划步骤已走完，但目标产出尚未完成，等待作者确认下一步。",
      target_tool_ref: nil,
      write_intent: :none,
      risk_hint: :medium,
  ...
  defp write_intent_for(_step, target)
       when target in ~w(character_design character_evolution plot_outline world_building prose_writing),
       do: :tentative
  ...
  defp execute_context_decision_step(run, sequence, spec, snapshot) do
    ...
    emit_stage(
      ...
      :goal_understood,
      "已组装当前作品上下文。",
      ["judgment_plan_context_assembled"],
      ["context:#{turn_id}"],
      %{
        stage: :judgment_plan_context_assembled,
    ...
    {:ok,
     %{
       ...
       progress_signature: "#{run.run_id}:judgment_plan_context:#{turn_id}"
     }}
  end
  ...
  defp do_execute_tool_step(run, sequence, spec, frame, plan, decision, observations, snapshot) do
    ...
    emit_stage(
      snapshot,
      :tool_started,
      "正在执行计划步骤：#{tool_name(plan)}。",
      ["judgment_plan_step_started"],
      [plan.plan_id, Map.get(decision, :decision_id)],
      %{
        stage: :judgment_plan_step_started,
    ...
    if Map.get(tool_result, :status) == :failed do
      ...
      emit_stage(
        ...
        :tool_completed,
        "计划步骤产出已生成。",
        ["judgment_plan_step_completed"],
        tool_refs(turn_result),
        %{
          stage: :judgment_plan_step_completed,
  ...
  defp context_step(run, sequence) do
    {:ok, step} =
      AgentStep.new(%{
        ...
        state_snapshot_ref: state_snapshot_ref(run, sequence, "judgment_plan_context"),
        idempotency_key: "#{run.run_id}:#{sequence}:judgment_plan_context:goal_v#{run.goal.version}"
      })
  ...
  defp tool_observation(run, sequence, frame, turn_result) do
    ...
    {:ok, observation} =
      AgentObservation.new(%{
        ...
        summary: "已生成 #{length(refs)} 个待采纳候选。",
  ...
  defp context_observation_id(run, sequence), do: "obs_#{run.run_id}_#{sequence}_jp_context"

  defp tool_observation_id(run, sequence), do: "obs_#{run.run_id}_#{sequence}_jp_tool"
