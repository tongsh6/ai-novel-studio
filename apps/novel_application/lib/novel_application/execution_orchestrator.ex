defmodule NovelApplication.ExecutionOrchestrator do
  ...
  @spec decide(DialogueFrame.t(), MicroPlan.t()) :: {OrchestratorDecision.t(), BehaviorState.t() | nil}
  def decide(%DialogueFrame{} = frame, %MicroPlan{} = plan) do
    ...
    {decision, behavior} =
      case PlannerBoundary.validate(frame, plan) do
        {:error, reason} ->
          {build_decision(:fail_with_recovery, decision_id, frame, plan, reason, "planner_boundary"), nil}
  ...
  defp decision_from_gate_result({:pass, _results}, decision_id, frame, plan) do
    if allow_tool_dispatch?(plan) do
      ...
      {build_decision(:downgrade_to_dialogue, decision_id, frame, plan,
        "gates passed but plan requires confirmation or is multi-step", "action_scope"), nil}
    end
  end
  ...
  defp open_behavior(decision_type, decision_id, frame, plan, reason) do
    ...
    behavior_type = if decision_type == :require_confirmation, do: :confirmation, else: :clarification
  ...
  defp behavior_actions(:clarification, d_id) do
    [
      %{action_type: "answer_clarification", behavior_ref: d_id, idempotency_key: "clarify_#{d_id}"},
      %{action_type: "cancel_pending_behavior", behavior_ref: d_id, idempotency_key: "cancel_#{d_id}"}
    ]
  end

  defp behavior_actions(:confirmation, d_id) do
    [
      %{action_type: "confirm_before_execute", behavior_ref: d_id, idempotency_key: "confirm_#{d_id}"},
      %{action_type: "reject_or_cancel_confirmation", behavior_ref: d_id, idempotency_key: "reject_#{d_id}"}
    ]
  end
  ...
  defp build_allow_decision(decision_id, frame, plan) do
    ...
    %OrchestratorDecision{
      decision_id: decision_id, turn_id: frame.turn_id, frame_ref: frame.frame_id,
      plan_ref: plan.plan_id, decision_type: :allow_tool, decision_status: :decided,
      approved_actions: action_refs(plan.proposed_actions), rejected_actions: [],
      downgraded_actions: [], required_author_action: nil, first_blocking_gate: nil,
  ...
  defp build_decision(decision_type, decision_id, frame, plan, reason, gate_name) do
    %OrchestratorDecision{
      decision_id: decision_id, turn_id: frame.turn_id, frame_ref: frame.frame_id,
      plan_ref: plan.plan_id, decision_type: decision_type, decision_status: :decided,
      approved_actions: [], rejected_actions: action_refs(plan.proposed_actions),
