defmodule Persistence.SchemaDrift do
  @moduledoc """
  ...
  alias Persistence.Schemas.Foundation.{ArtifactAdoptionEntry, TurnResult}
  ...
  defp compare_loaded(module, json, json_path) do
    json_props = json |> Map.get("properties", %{}) |> Map.keys() |> Enum.map(&String.to_atom/1) |> MapSet.new()
  ...
  defp diff(json_set, elixir_set, label) do
    ...
    msgs =
      if MapSet.size(only_elixir) > 0,
        do: msgs ++ ["#{label}: in Ecto but not in JSON: #{inspect(MapSet.to_list(only_elixir))}"],
        else: msgs
