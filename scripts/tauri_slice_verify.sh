...
#   bash scripts/tauri_slice_verify.sh gap-wt04-non-exploration-frame-badges
...
set -euo pipefail
# Background Phoenix/Tauri jobs must own process groups so the shared
# process-tree cleanup can reap their real beam/vite/native descendants.
set -m
...
if [[ "$SLICE_ID" != "au14-fact-inventory-roundtrip" && "$SLICE_ID" != "au14-finding-inventory-arc-loop" && "$SLICE_ID" != "au14-assumption-confirm-roundtrip" && "$SLICE_ID" != "au14-assumption-provisional-injection" && "$SLICE_ID" != "gap-wt04-non-exploration-frame-badges" && "$SLICE_ID" != "au08-volume-structured-planning" && "$SLICE_ID" != "au12-character-identity-merge" && "$SLICE_ID" != "au14-foreshadow-resolution-roundtrip" ]]; then
...
native_action_description() {
  case "$SLICE_ID" in
    ...
    au01-ordinary-chat-two-turn-roundtrip)
      ...
      ;;
    gap-wt04-non-exploration-frame-badges)
      echo "open real workbench -> send a question-answer turn and a meta-discussion turn -> verify distinct visible frame badges inside the assistant rail without MicroPlan, action, adoption, or production write"
      ;;
    au01-empty-message-guard)
    ...
    p1-chapter-plan-minimum)
      ...
      ;;
    au08-volume-structured-planning)
      echo "seed a work whose skeleton declares 2 planned volumes -> open real archive outline -> click start planning -> the plan comes back with a per-chapter 所属卷 assignment -> adopt -> reopen archive outline and verify two volume headers (卷名 · N章) with the chapters grouped exactly as the adopted plan assigned them -> open reading mode and verify the TOC shows the same two volume headers while get_toc reports volume_count=2"
      ;;
    p1-chapter-draft-generation)
    ...
    agent-provider-execution-error-author-safe)
      ...
      ;;
    agent-provider-cancel-honest-boundary)
      echo "send an author-reachable slow prose provider step -> confirm termination -> verify run_id binding, cancelling state, provider execution cancel request, and terminal cancelled state"
      ;;
    agent-readonly-batch-profile)
    ...
    agent-prose-drafting-with-quality)
      ...
      ;;
    p1-prose-companion-artifacts)
      echo "send one prose request from the real workbench -> verify prose plus character/foreshadow/world-rule/constraint candidates share one provider call, remain tentative, and selective character adoption reaches the archive"
      ;;
    wr01-chapter-mission-before-prose)
      echo "seed plan + ledgers -> send chapter-3 continuation from the real workbench -> verify the model plan schedules chapter_mission before prose_writing, the reasoning area shows 本章使命, mission basis refs stay inside the selected materials, the brief sources from chapter_mission, and no production write happens"
      ;;
    wr01-chapter-mission-author-decision)
      echo "write chapter 3 (mission stored as TENTATIVE on the chapter plan) -> open archive outline, rewrite the mission in place -> write chapter 3 again -> verify the author version is used with zero mission provider calls, the brief sources from it, and nothing is written to production"
      ;;
    wr02-planning-mission-before-outline)
      echo "seed plan + ledgers -> ask the real workbench to plan the next chapters -> verify the mechanical step order runs planning_mission before plot_outline, the reasoning area shows the planning mission narrative, basis refs stay inside the selected materials, and the outline draft stays tentative with no production write"
      ;;
    ca03-carry-registry-observability)
      echo "send one prose request and one planning request from the real workbench -> verify each creative call logs context.carry.done whose carried/gated/empty split matches the carry registry for its capability, and both artifacts stay tentative with no production write"
      ;;
    ca04-carry-gap-closure)
      echo "seed skeleton + confirmed facts/style + protagonist -> drive prose, planning and world_building from the real workbench -> verify the three newly approved carriers are actually carried (prose: work_skeleton; planning: creative_facts/style_guide; world_building: character_roster) with tentative artifacts and no production write"
      ;;
    wr01c-planning-mission-decision)
      echo "plan next chapters (mission stored as TENTATIVE on the work) -> open archive outline, rewrite the work-level mission in place -> plan again -> verify the author version is used with zero mission provider calls, the why dialog shows the structured mission block, and nothing is written to production"
      ;;
    d1-character-vacuum-to-named-roster)
      echo "seed a work with zero characters -> first prose request carries the vacuum naming directive and emits a companion character_seed -> adopt it from the candidate card -> second prose request carries the adopted name in 现有角色 roster with the directive switched to unmarked-protagonist"
      ;;
    d3-summary-lazy-repair)
      echo "adopt chapter 1 whose summary generation fails async (D3SUMFAIL) -> chapter-2 assembly read schedules lazy repair -> repair completes -> a following prose prompt carries the repaired chapter-1 summary; adoption main chain never blocked"
      ;;
    d4-planner-transient-retry)
      echo "send a chapter-3 prose request carrying D4PLANFAIL -> the plan draft first call fails transiently -> the planner retries the same prompt once and succeeds -> run completes with a tentative prose_fragment and plan_draft.retry.start logged"
      ;;
    d6-purpose-model-routing)
      echo "configure a writer purpose override through the public provider config API -> open real model settings and see the per-purpose section defaulting to follow-global -> send a prose request -> the writer provider call logs the override model while other calls keep the global model"
      ;;
    agent-conversation-turn)
    ...
    p1-chapter-word-count-target)
      ...
      ;;
    au13-arc-ledger-roundtrip)
      echo "seed adopted chapter plan + adopted character roster -> generate + adopt chapter-1 prose (roster character woven) -> arc ledger auto-updates (ledger.update.done) -> ask the ledger question in chat -> judgment explores archive_read(ledgers) and the reply cites the real arc entry visibly -> chapter-2 prose request carries the progress_state projection (context.progress_state.done)"
      ;;
    au13-review-adjudication-roundtrip)
      echo "seed 4 adopted characters with STALLED arc entries plus a target length and a 第一卷终局 chapter-4 plan -> threads panel readonly view (five threads + honest empty report) -> click full-review action to start the real ledger_reconciliation_v1 run -> report materialized with 5 findings (4 arc stalled + VS-00G R7 premature finale carrying progress percent and chapter seq) -> adjudicate each finding with a distinct disposition (accept_drift / dismiss evidence log / revise_design + revise_prose correction intents via real user_message, premature finale via revise_design) -> report turns fully dispositioned"
      ;;
    au13-revise-prose-sibling)
      echo "seed adopted chapter-1 prose + STALLED arc entry -> full review report -> revise_prose disposition sends the rewrite correction intent -> high-risk confirm -> sibling revision candidate stays pending prose_fragment -> reading projection still shows the original seeded prose (original preserved)"
      ;;
    au14-fact-inventory-roundtrip)
      echo "seed accepted chapter prose with empty setting archives -> open the real archive -> click fact inventory -> fact_inventory_v1 reads accepted material and returns character/rule/foreshadowing/planning tentative candidates -> adopt one character and one rule -> pick 目标体量 in the multi-suggestion planning candidate group and verify the accept button says 采纳方案 A 为全书规划 (never 保存…到作品档案) -> verify only adopted items enter archive projections while the rest stay pending -> run fact inventory again so the model re-proposes the already accepted 沈砚 -> adopting the same name raises require_confirmation with a card that names the existing role and explains 别名/改名 -> archive still holds one 沈砚 row until the author confirms, then two"
      ;;
    au14-finding-inventory-arc-loop)
      echo "seed 10 accepted chapters with an empty character archive -> full review materializes protagonist_undermaterialized -> click the finding-bound fact inventory action -> adopt the proposed protagonist -> generate and adopt chapter 11 -> verify the protagonist arc ledger starts without history backfill"
      ;;
    au14-assumption-confirm-roundtrip)
      echo "seed accepted chapter prose with an empty character archive -> start fact inventory from the archive overview -> completion auto-materializes the protagonist as a provisional assumption with a visible notice -> overview shows the provisional section with badge, protagonist row, and confirm/discard actions -> click confirm -> assumption converts in place into exactly one accepted character and the provisional section disappears"
      ;;
    au14-assumption-provisional-injection)
      echo "seed 10 accepted chapters with an empty character archive -> start fact inventory to activate the provisional protagonist assumption -> generate the next chapter and verify mechanical preparation counts the active assumption as protagonist presence (design_missing without protagonist, provisional prompt marker) -> discard the assumption from the overview -> generate another chapter and verify the protagonist absence directive is restored"
      ;;
    au12-character-identity-merge)
      echo "seed duplicate same-name character rows plus an alias row with two arc ledger entries (m4b specimen shape) -> open the real archive character tab -> author merges the duplicate row then the alias row through the merge dialog (target choice, keep-name choice, consequence copy) -> roster collapses to a single row showing the absorbed alias -> threads panel arc summary unifies to a single subject -> merging performs no adoption/prose/memory write -> fact inventory re-proposes the merged alias as a new character -> adopting it hits the alias-aware duplicate confirmation naming the canonical row -> author rejects and the archive stays a single aliased row with zero accepted adoption"
      ;;
    au14-foreshadow-resolution-roundtrip)
      echo "seed a three-chapter plan with two written chapters and an adopted foreshadow expected to resolve by chapter 1 (overdue) -> threads panel shows the hidden foreshadow with zero arc entries -> full review reports foreshadowing_overdue naming the expectation -> a chapter-3 prose request injects the information progress_state section (zero arc entries prove the info section) -> fact inventory checks the unresolved list and proposes a resolution anchored to the ledger ref -> author clicks 确认伏笔已回收 -> ledger entry turns REVEALED through the adoption boundary and the threads panel shows 已揭示"
      ;;
    judgment-explore-internal)
      echo "seed adopted chapter plan -> generate + adopt a chapter-1 prose draft -> ask a prose-only fact question in chat -> judgment decides explore -> prose_search retrieves adopted prose -> reply cites the fact with its source chapter"
      ;;
    judgment-explore-chapter-plan)
      echo "seed adopted chapter plan (no prose) -> ask what a chapter is planned to cover -> judgment decides explore -> chapter_read retrieves design-state plan -> reply cites the plan content with its chapter"
      ;;
    au04-confirm-before-execute)
    ...
    au09-memory-list-ux-redesign)
      ...
      ;;
    au09-character-candidate-per-item-adoption)
      echo "design two character candidates in one turn -> each candidate gets its own selector and action target -> select and adopt only the second candidate -> the first candidate stays selectable and never enters the work -> archive character tab shows only the adopted candidate"
      ;;
    au12-archive-concurrent-model-run-read-snapshot)
...
if curl -s --max-time 1 "$API_URL/health" >/dev/null 2>&1; then
  echo "[tauri-slice-verify] Phoenix port is already occupied: $PHOENIX_PORT" >&2
  exit 1
fi
if curl -s --max-time 1 "http://127.0.0.1:${VITE_PORT}" >/dev/null 2>&1; then
  echo "[tauri-slice-verify] Vite port is already occupied: $VITE_PORT" >&2
  exit 1
fi
reset_test_db
...
case "$SLICE_ID" in
  ...
  au05-discard-author-action)
    ...
    ;;
  au13-arc-ledger-roundtrip)
    SEED_SCRIPT="scripts/seed_au13_arc_ledger.exs"
    ;;
  au13-review-adjudication-roundtrip)
    SEED_SCRIPT="scripts/seed_au13_review_adjudication.exs"
    ;;
  au13-revise-prose-sibling)
    SEED_SCRIPT="scripts/seed_au13_revise_prose.exs"
    ;;
  au14-fact-inventory-roundtrip)
    SEED_SCRIPT="scripts/seed_au14_fact_inventory.exs"
    ;;
  au14-finding-inventory-arc-loop)
    SEED_SCRIPT="scripts/seed_au14_finding_inventory_arc.exs"
    ;;
  au14-assumption-confirm-roundtrip)
    SEED_SCRIPT="scripts/seed_au14_fact_inventory.exs"
    ;;
  au14-assumption-provisional-injection)
    ...
    ;;
  au08-volume-structured-planning)
    SEED_SCRIPT="scripts/seed_au08_volume_planning.exs"
    ;;
  au12-character-identity-merge)
    SEED_SCRIPT="scripts/seed_au12_character_merge.exs"
    ;;
  au14-foreshadow-resolution-roundtrip)
    SEED_SCRIPT="scripts/seed_au14_foreshadow_resolution.exs"
    ;;
  p1-chapter-draft-generation)
    SEED_SCRIPT="scripts/seed_p1_chapter_draft_generation.exs"
    ;;
  ...
  p1-prose-revision-candidate)
    ...
    ;;
  quality-revision-action-run-anchoring)
    SEED_SCRIPT="scripts/seed_quality_revision_action_run_anchoring.exs"
    ;;
  p1-prose-quality-finding-roundtrip)
    SEED_SCRIPT="scripts/seed_p1_prose_revision_candidate.exs"
    ;;
  ...
  p1-prose-companion-artifacts)
    ...
    ;;
  wr01-chapter-mission-before-prose)
    SEED_SCRIPT="scripts/seed_wr01_chapter_mission.exs"
    ;;
  wr01-chapter-mission-author-decision)
    SEED_SCRIPT="scripts/seed_wr01_chapter_mission.exs"
    ;;
  ...
  ca03-carry-registry-observability)
    ...
    ;;
  ca04-carry-gap-closure)
    SEED_SCRIPT="scripts/seed_ca04_carry_gaps.exs"
    ;;
  wr01c-planning-mission-decision)
    SEED_SCRIPT="scripts/seed_wr01_chapter_mission.exs"
    ;;
  d1-character-vacuum-to-named-roster)
    SEED_SCRIPT="scripts/seed_d1_character_vacuum.exs"
    ;;
  d3-summary-lazy-repair)
    SEED_SCRIPT="scripts/seed_d3_summary_repair.exs"
    export NOVEL_SYNC_SUMMARY_MAINTENANCE=false
    ;;
  d4-planner-transient-retry)
    SEED_SCRIPT="scripts/seed_wr01_chapter_mission.exs"
    ;;
