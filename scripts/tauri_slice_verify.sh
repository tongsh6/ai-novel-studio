...
if [[ "$SLICE_ID" != "au14-fact-inventory-roundtrip" && "$SLICE_ID" != "au14-finding-inventory-arc-loop" && "$SLICE_ID" != "au14-assumption-confirm-roundtrip" && "$SLICE_ID" != "au14-assumption-provisional-injection" && "$SLICE_ID" != "gap-wt04-non-exploration-frame-badges" && "$SLICE_ID" != "au08-volume-structured-planning" ]]; then
