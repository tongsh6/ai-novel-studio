...
native_action_description() {
  case "$SLICE_ID" in
    ...
    au13-arc-ledger-roundtrip)
      ...
      ;;
    au13-review-adjudication-roundtrip)
      echo "seed 4 adopted characters with STALLED arc entries -> threads panel readonly view (five threads + honest empty report) -> click full-review action to start the real ledger_reconciliation_v1 run -> report materialized with 4 findings -> adjudicate each finding with a distinct disposition (accept_drift / dismiss evidence log / revise_design + revise_prose correction intents via real user_message) -> report turns fully dispositioned"
      ;;
    au13-revise-prose-sibling)
