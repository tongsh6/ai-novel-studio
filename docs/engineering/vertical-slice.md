...
- `Exploration` 必须回答本 slice 物化的要素/数据经探索面（判断循环内部翼工具目录）如何**同批可达**（[08 §8 探索面同步律](../design/08-novel-element-model.md)）。要素落地与探索可达必须同批收口，否则账本建了 AI 查不到，等于没建。非要素物化 slice 可答“不适用”并说明理由（例如纯 UI 呈现、纯基础设施）。
  > 沿革：原为六问（探索可达作为 Consumer 项内的“探索面同步律”附加要求，2026-07-19 CP5 教训）；2026-07-21 升格为独立第七问。历史 slice 文档中的“承重六问”为当时口径，不回溯改写。
