...
> 状态：草案
>
> 角色：当前 UI 设计的共同原则文档。后续 `41`-`47` 与 `novel-studio.pen` 必须引用本文，不得反向发明 Foundation / Domain 语义。
...
| 来源 | 本文消费的语义 |
| --- | --- |
| `../README.md` | 当前唯一设计入口和设计真源边界 |
| `../00b-end-to-end-dialogue-flow.md` §2-§5 | 用户输入到 adoption / projection 的主链路 |
| `../00d-runtime-architecture.md` | Workbench / Reading View / Structure Panel 的运行时位置 |
| `../07-workbench-ui-contract.md` §2-§8 | UI 只能投影 TurnResultViewModel、assistant message、ui_cards、available_actions、projection hints |
| `../contracts/VS-05-ui-roundtrip-contract-pack.md` §2-§4 | TurnResultViewModel、AvailableAction roundtrip、UI card type 最小集合 |
| `../contracts/VS-04-adoption-boundary-contract-pack.md` | 候选选择、采纳边界和 projection refresh |
| `../contracts/VS-03-behavior-lifecycle-contract-pack.md` | clarification / confirmation / recovery 等等待作者状态 |
| `../adr/ADR-0015-turn-result-view-model-v3.md` | TurnResultViewModel 作为 UI 唯一主消费 envelope |
| `../adr/ADR-0016-projection-hint-ui-v3.md` | projection hint 与阅读投影刷新 |
| `../domain/34-novel-element-field-priority.md` | 结构面板字段优先级与渐进披露 |
...
当前 UI 面向 **中文作者**，核心场景是 **中国网文 / 长篇连载创作**。
...
该约束适用于 `41`-`47`、`novel-studio.pen` 以及所有导出图。
...
本文对应 `novel-studio.pen` 中的整体设计方向，不单独要求一个独立 screen。
