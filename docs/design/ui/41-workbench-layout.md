...
- **双向文本轨道**：普通作者文本右对齐并持续保留左侧留白，普通 assistant 文本左对齐并持续保留右侧留白；两者必须通过克制的背景、轮廓和角色标签形成可辨识差异。AgentRun 内作者可见的模型叙事段沿用 assistant 阅读宽度；结构状态、`ui_cards`、候选产物和固定控制坞仍可使用主操作轨道宽度。
- **两种内容宽度**：小说正文、普通对话、模型叙事，以及由 AI 回复直接展开的探索方向/讨论建议保持约 `680–760px` 的同一阅读宽度；同一 assistant 回合内即使文本较短，正文、过程叙事和探索方向也保持相同列宽与左右边缘。质量卡、需要横向比较的候选产物、任务控制等操作型组件允许使用 `820–880px`。两者共享居中轨道，但不得把自然语言或探索性 AI 输出简单拉满到操作宽度，也不得出现向右凸出的第三种宽度。
- **候选讨论渐进收束**：候选方向首次出现时完整展开。作者点击“继续讨论”且服务端接受该 `user_message.candidate_selection` 后，来源候选组折叠成“已沿「候选标题」继续讨论”的单行摘要，并保留“展开”入口；发送失败时保持展开。会话刷新后应从 transcript 中持久化的 `candidate_selection` 恢复默认折叠态，而不是依赖仅存于当前进程的临时布尔值。折叠只收束来源候选组，不隐藏后续 AI 新生成的候选，也不把 selection 视觉误写成 adoption。
- **单一主叙事**：同一 assistant 回合内，`assistant_message.text` 与 AgentRun `author_narrative` 若出现逐字相同的完整段落，只显示一次；`assistant_message` 保留为作者主回复，AgentRun 只补充不重复的过程叙事与结构状态。不得用模糊语义匹配改写、拼接或吞掉不同来源的正文。
- **普通回复降噪**：角色标签已经表达 AI 身份时，`casual_reply` 不再额外显示“AI 回应”frame badge。普通聊天的 bounded run 若已成功完成，且没有独立过程叙事、计划、待采纳产物、正文草稿或 author_action 来源，则回复本身已经构成完成反馈，不再追加“已完成 · 创作执行”终态摘要；失败/取消、长任务、作者动作、计划和产物状态不得因此隐藏。
...
- **收起轨道只表达档案**：轨道入口使用档案图标、短标签“档案”和待采纳数字 badge；会话历史属于顶栏作品上下文入口，不与作品对象混排。
...
1. 当前作品 / 卷 / 章。
2. 当前模式：创作态 / 阅读态入口。
3. 活跃任务状态：优先聚合真实 `AgentRun`（运行中 / 暂停中 / 已暂停 / 等待确认 / 终止中），其次才显示 long-run/checkpoint 摘要；存在活跃或暂停 run 时不得显示“无任务”，详情回到主对话 turn 与固定控制坞。
...
输入区通过留白和输入框自身轮廓与对话流分层，不增加横贯主工作区的顶部装饰分隔线。
