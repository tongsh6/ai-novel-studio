...
### 5.0.3 本章使命的裁决面（2026-08-22 用户拍板，WR01b）

写前推理层（VS-00E §16）产出的「本章使命」落在章计划上（`plan_direction.chapter_mission`），
裁决入口按拍板放在**大纲与结构 tab 逐章**，与「暂定设定」同款心智（系统可写暂定、
转正只经作者动作），不做候选卡、不经采纳边界：

- **展示**：每章卡片在摘要之下显示使命块——状态徽标（【暂定】/【已确认】/【作者改写】）、
  一句使命、「必须推进 / 不得」条目（模型版条目附依据文本）。无使命的章不显示该块。
- **动作**（文案与暂定设定的「确认/否决」区分）：
  - 【暂定】：`确认使命` / `改写使命` / `作废使命`；
  - 【已确认】【作者改写】：`改写使命` / `作废使命`。
- **改写**：就地编辑（一句使命 + 必须推进多行 + 不得多行）→「保存改写」/「取消」；保存即
  `AUTHOR_EDITED`。
- **后果语义**：作者版在场时下次写这章**不再让模型推**、不被覆盖；作废后下次写作重新推导。
  使命是设计态，不进作品事实、不进记忆与账本。
- 数据来源：`get_toc` 投影已带 `plan_direction`；动作走 `author_action`（ADR-0024 S8），
  成功后面板重读 TOC（真实读端口，非本地拼装）。

### 5.0.4 工作级规划使命的裁决面（2026-08-24 用户拍板，WR01c）

「大纲与结构」tab 顶部（骨架进度行之前）渲染**工作级规划使命块**（`get_toc` 顶层
`planning_direction.planning_mission`）：徽标（【暂定】/【已确认】/【作者改写】）+ 一句使命 +
必须推进/不得逐条（带依据标签）+ 三动作（确认/改写/作废，`*_planning_mission` author_action，
work 级 payload 无 ref）。改写为就地三 textarea 编辑，语义与 §5.0.3 章使命块完全同构——
一个管「这一章怎么写」，一个管「接下来规划什么」。确认/改写后下轮规划 0 调用直取作者版；
作废后下轮重新推导。契约：VS-00E §16.10 / ADR-0024 S9。
