...
- `../domain/27-reading-projection.md`：阅读投影的生成规则。
- `../contracts/VS-04-adoption-boundary-contract-pack.md`：accepted source、candidate selection 与 adoption/projection 边界。
- `../adr/ADR-0016-projection-hint-ui-v3.md`：ProjectionHint 与投影刷新提示。
- `../07-workbench-ui-contract.md` §7 / §8：projection hint refresh only，不授权 UI 写入。
...
2. 必须明确使用并展现阅读投影规定的 `FRESH` / `STALE` / `REBUILDING` / `FAILED` 四个状态。
