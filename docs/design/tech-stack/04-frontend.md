...
> 状态：草案
>
> 目的：定义 React/TS 前端栈、关键组件、与后端 contract 的投影方式。本文档不展开具体 UI 设计（那些在 `../ui/` 目录），只回答"前端用什么技术实现"。
...
[`../00d-runtime-architecture.md`](../00d-runtime-architecture.md) 的运行时边界与 [`../07-workbench-ui-contract.md`](../07-workbench-ui-contract.md) §2 共同约束：UI 不能发明语义。具体到前端代码：

- 卡片类型必须从 `../contracts/VS-05-ui-roundtrip-contract-pack.md` §4 当前集合中选（`candidate_set` / `clarification_prompt` / `confirmation_request` / `recovery_prompt` / `trace_summary` / `projection_notice` / `capability_notice`）。
- 可提交动作必须来自同一 TurnResultViewModel 的 `available_actions`，`action_type` 使用 `../07-workbench-ui-contract.md` §5 当前集合。
- `card.status` 不作为前端自造状态；卡片状态必须从 TurnResult、behavior、task、adoption 或 projection state 投影。
- 不能在前端造新 intent，必须由 planner / orchestrator 决策。

### 3.2 schema codegen，不手写

后端 `docs/design/schemas/*.json` 是 SSOT，前端用 codegen 生成 Zod schema + TS type。详见 [`09-schema-codegen.md`](./09-schema-codegen.md)。
...
`../00d-runtime-architecture.md` 的运行时边界：UI 不能直接读 Domain Stores。
...
│   │       └── projection.ts
│   ├── generated/
│   │   ├── schemas/                     # codegen from docs/design/schemas/
│   │   │   ├── turn_result.ts
│   │   │   ├── card.ts
│   │   │   ├── adoption_state.ts
│   │   │   └── ...
...
│   ├── components/
│   │   ├── cards/                       # docs/design/ui/42 + VS-05 card system
...
## 6. 与 docs/design/ui/ 文档的关系

`docs/design/ui/40-47.md` 是 **UI 语义层**（设计契约）。
...
以上 TBD 在 UI 实现阶段开始时确定，不阻塞后端 contract 与当前设计真源维护。
