...
- `2026-07-15-judgment-driven-interactive-loop.md` — 判断驱动交互循环（用户方向拍板）：对话/交互式创作从"计划驱动"改为"机械准备 + 模型判断① / ② + 用户裁决出口"的真循环，计划驱动收窄为长任务路线图域（N-PLAN 适用域改写）；判断协议三方案对比（推荐 B：判断两段式 + 直接回复内联，简单对话 2 次调用）；与 ADR-0021/0022/0023、S1-S7、D 系、N-NARR 的资产映射与 CP0-CP5 路线；§5a 探索能力域（用户确认必须：检索工具箱 + 作品事实索引 + 探索预算，"AI 判断我需要什么后去拿"）。作为 ADR-0025 输入。
