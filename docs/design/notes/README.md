...
- `2026-08-26-m6-gptoss-contrast.md` — M6 对照狗粮报告（gpt-oss-120b vs qwen3.8-27b，同任务同刻度唯一变量=模型）：12 章 18181 字 30 分钟一次收官、201 调用全 200 全 purpose completed、writer 单调用中位 24.2s（qwen 532s 的 1/22）、盘点 1/1 成功 16 提案主角建档（qwen 3/3 败）、元词泄漏 0；D6 路由表填表建议（盘点/规划/评审→gpt-oss 无悬念，写作待两书样章盲评）。**不授权实现**。
