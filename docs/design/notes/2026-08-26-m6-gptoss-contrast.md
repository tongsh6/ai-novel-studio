...
## 4g. 结论定位更正：这不是「模型定版」问题（2026-08-26 作者纠正）

初版 §4g 曾把结果写成「模型定版三选一，待作者拍板」——**框架错了，已撤回**。作者纠正：

1. **产品本就不绑模型**：SU-01 设置页可切供应商与模型，D6 又提供按用途路由；「选哪个
   模型」是作者/用户的运行时选项，不是需要在设计侧拍板固化的东西。
2. **违背既有地板档原则**：本项目明确「开发用本地小模型 = 刻意的 forcing function，
   不是设计基准；组装语义模型无关、预算按 provider profile 参数化；勿按最弱模型做设计
   判断」。把开发期跑分偏好上升成产品决策，正是该原则要防的漂移。

**因此这批对照跑（M6-M10c）的正确定位是三条，都不含「选型」**：

- **产品缺陷发现（真正的产出）**：D7 续写意图破坏性默认。它对**任何**「不总填可选
   字段」的模型都会发作，与具体模型无关；gpt-oss 恒填只是恰好掩盖了它两个月。
- **模型无关性的正面证据**：同一套主链在 gpt-oss-120b（MoE）、qwen3.8-27B（dense，
   Q4/Q8/BF16 三量化 × none/medium/xhigh 三思考档）下都能跑通全书并成功盘点——这正是
   地板档原则想要的结果，值得作为「组装语义模型无关」的实证登记。
- **开发期运行手册（工程便利，非设计约束）**：狗粮长跑用 gpt-oss-120b 周转 30 分钟、
   qwen-BF16 约 4 小时；BF16×xhigh 会撞产品 900s 护栏不可用。选哪个跑狗粮按当次要验
   什么定，随时可换，不写死。

文学质感盲评仍可做，但它是**内容侧观察**，与产品选型无关。
