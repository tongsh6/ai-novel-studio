...
>
> 2026-06-22 三轮 security checkpoint：新增 `e2e-01-channel-action-security --surface tauri` 外部协议 fuzz driver。driver 先打开真实 Tauri workbench，再用外部 Phoenix socket 加入同一 work/session，发送两轮普通 `user_message` 建立服务端持有的 TurnResult，然后提交伪造 `source_turn_result` 的 invented action 与旧 `source_turn_ref` 的 stale forged action。两者均被 Channel/application 授权边界拒绝，且无 `action_result` broadcast、无 `channel.author_action.done`、无产品验收 hook。当前状态更新为 **12/13 已验收、1/13 已测试、0/13 部分实现**；E8/E13 P2 Channel security regression 已关闭，E10 仍作为 AU-07 cross-reference。
>
> 2026-06-22 AU-07 cross-reference 补充：`au07-gate-reason-why --surface tauri --provider lmstudio` 已通过真实 Tauri + real LM Studio 验收，补强 E4/E10 的解释链路：真实档案入口触发多步 MicroPlan 后 Orchestrator 在 `action_scope` 降级，作者点击消息流“为什么”可见中文 gate/reason、no-provider replay 和 no-write 证据，且无 raw/internal code 泄漏。随后 `au07-trace-query-scope-negative-matrix --surface tauri` 通过，补强 E10 trace query scoped negative proof：原 work/session/turn replay 200，跨 work、跨 session、same-work other session 和 missing turn 均 404 且不泄露 trace。E10 的 developer view、多类型 trace query UI 仍归 AU-07 后续，不在 E2E-01 内伪关闭。
...
| E4 执行降级 | 已验收 | `artifacts/slice-verify/e2e-01-downgrade-real-page-tauri-lmstudio/summary.json`：真实 Tauri 档案面板点击“发起新操作”→ LM Studio 生成 3 action MicroPlan → Orchestrator 在 `action_scope` 降级；无 `toolbox.execute`、无 `author_action`、无执行控件、无 production write；UI badge 显示“降级为对话”；`artifacts/slice-verify/au07-gate-reason-why-tauri-lmstudio/summary.json` 进一步证明该降级可从真实 why 面板展示中文 gate/reason 且 no-provider/no-write | `v3_full_chain_test.exs` multi-step plan → `downgrade_to_dialogue`；`framePresentation.test.ts` 覆盖 downgrade badge；`trace_replay_service_test.exs` 持久 replay summary 恢复白名单 gate | 无阻塞 | closed |
...
| E8 Action 校验 | 已验收 | `artifacts/slice-verify/e2e-01-channel-action-security-tauri/summary.json`：真实 Tauri 页面锚定同一 work/session，外部协议 socket 发送伪造 `source_turn_result` 的 invented action 和 stale forged action；两者均产生 `channel.author_action.error`，无 `action_result`、无 `author_action.done`。`au04-stale-confirmation-ui`、`au04-disabled-confirmation-action-ui`、`au06-single-active-confirmation` 另证明 stale/disabled/old action 在真实页面不可执行或被拒绝 | `workspace_channel_v3_test.exs` invented/stale/source validation | 无阻塞 | closed |
...
| E10 持久化闭环 | 已测试 | 无独立 E2E trace query UI；why/trace UI 证据归 AU-07，E9 ReplayReport 已覆盖持久 trace 到结构化回放；`au07-gate-reason-why` 已补 action_scope 降级 why 的持久 replay summary 回填证据；`au07-trace-query-scope-negative-matrix` 已补 scoped negative matrix 证据 | `dialogue_gateway_real_loop_test.exs` 2026-06-21 新增 `handle_input` + `WorkspaceContext.trace_persister` + `TraceRepository.list_by_turn` 回查；`trace_repository_test.exs`；`trace_replay_service_test.exs` | application/persistence 链路已闭合；developer / 多类型 trace query UI 继续归 AU-07 | cross-reference：AU-07 |
...
| E13 Action 来源校验 | 已验收 | `artifacts/slice-verify/e2e-01-channel-action-security-tauri/summary.json`：真实 Tauri 页面先建立 work/session，外部协议 socket 在同一 Channel 会话内证明客户端伪造的 `source_turn_result` 不能授权 invented action，旧 `source_turn_ref` 的 forged action 在 current turn 推进后被 stale 拒绝；合法 action 授权仍由 AU04/AU05/AU06 真实页面证据覆盖 | `workspace_channel_v3_test.exs` exact：客户端伪造 `source_turn_result` 不能授权 invented action | 无阻塞 | closed |
...
bash scripts/quality_accept.sh e2e-01-channel-action-security --surface tauri
```
...
2026-06-22 security checkpoint 结果：`quality_accept e2e-01-channel-action-security --surface tauri` 通过并写入 `artifacts/slice-verify/e2e-01-channel-action-security-tauri/summary.json`。首次沙箱内运行因 Mix PubSub TCP `:eperm` 在启动前失败；提升权限后同一入口通过。后续 `e2e-01-full-chain --provider lmstudio` 聚合 runner 消费该 summary 后，`scenario_status_counts` 更新为 `12/13 已验收、1/13 已测试`，`remaining_gaps` 仅保留 E10 的 AU-07 cross-reference。
...
- P2：已关闭。E8/E13 的 invented / forged source negative 已由外部协议 fuzz harness `e2e-01-channel-action-security` 关闭；该 harness 以真实 Tauri 页面作为 work/session anchor，不在产品 UI 添加构造入口或验收感知逻辑。
- cross-reference：E10 普通旧 turn scoped query、partial replay 和 scoped negative matrix 已由 AU-07 回填；developer / 多类型 trace query UI 继续归 AU-07 owner，不阻塞 E2E-01。
...
注意：该 runner 是文件级证据聚合器，不是产品页面内逻辑，也不为产品新增验收感知 env/query/localStorage、DOM hook 或自动采纳行为。E4、E6、E8、E9 与 E13 已由独立真实 Tauri checkpoint 升级为“已验收”；E10 普通旧 turn scoped query、partial replay 和 scoped negative matrix 已由 AU-07 回填，developer / 多类型 trace query UI 深化继续作为 AU-07 cross-reference。
