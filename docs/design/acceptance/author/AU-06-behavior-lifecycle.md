...
>
> 2026-06-22 二轮缺口复核：AU-06 关联的 12 个真实 Tauri / quality 入口已串行复跑通过：`au04-confirm-before-execute`、`au04-confirmation-tool-failure-recovery`、`au04-confirm-idempotency-ui`、`au04-stale-confirmation-ui`、`au06-single-active-confirmation`、`au04-confirmation-ttl-ui`、`au04-disabled-confirmation-action-ui`、`au04-history-confirmation-readonly`、`au04-cross-work-confirmation-guard`、`au04-latest-context-rebase-confirmation`、`au10-workbench-recovery-cancel-waiting`、`au07-behavior-trace-terminal-replay`。本轮未发现 AU-06 文件内必须关闭的新 P1/P0；取消等待后的 terminal BehaviorTrace 已由 AU-07 cross evidence 证明，confirm terminal replay、blocking clarification、持久 BehaviorBinding ledger 继续登记为后续 P1。
...
| 编号 | 不变量 | 当前验证 |
|---|---|---|
| AU06-I1 | Durable behavior 必须有 open/resolving/resolved/cancelled/failed/superseded 生命周期 | cancel terminal history 与 terminal BehaviorTrace 已验收；confirm terminal history / replay 为 P1 |
...
| AU06-I9 | Behavior lifecycle 必须可 trace/replay，且 replay 不调 LLM | cancel terminal BehaviorTrace 已由 AU-07 cross evidence 覆盖；完整 confirm replay / 旧 turn 查询为 P1 |
...
| SC-AU06-D1 行为生命周期可回放且不调 LLM | ReplayService 从 trace/history 还原 open/resolving/closed，不调用 LLM | VS-03 proof；AU06-I9 | `ReplayService`、trace summary、interaction recorder、TraceReplayService | AU-07 局部 replay 证据 | `au07-behavior-trace-terminal-replay`：cancel waiting 后 cancelled action turn 记录 terminal BehaviorTrace close/resolution refs，且 no-provider/no-tool/no-write；`au07-persisted-trace-query`：普通旧 turn scoped replay query 已闭合；`au07-partial-replay-ui`：partial 作者提示已闭合 | 部分实现 | cancel terminal replay producer 已补；普通旧 turn query 与 partial 作者提示已闭合；confirm terminal replay、完整 BehaviorTrace replay 和 developer 双视图仍未闭环 | 补集成/补验收 | P1 | AU-07 文件 owner |
...
| AU06-GAP-04 — open -> resolving -> resolved/history 未闭环 | cancel terminal history 与 terminal BehaviorTrace 已关闭；confirm terminal history / replay 仍缺 | 补集成/补验收 | P1 | AU-07 replay / BehaviorTrace |
...
| AU06-GAP-10 — BehaviorTrace / replay 缺失 | cancel terminal BehaviorTrace 已由 AU-07 cross evidence 覆盖；完整 confirm replay / 旧 turn 查询仍登记 AU-07 owner | 补集成/补验收 | P1 | AU-07 |
...
| 已实现场景有局部测试证据 | 是，见 §6 |
| 承重主链有外部自动化驱动真实页面证据 | 是，12 条相关 Tauri / quality_accept driver |
