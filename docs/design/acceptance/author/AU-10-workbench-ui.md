...
**当前状态**：WebSocket service reconnect CP2、cancel waiting CP3A 与 provider timeout CP3B 已闭环 / 完整 LongRunner 恢复未闭环。
...
**当前状态**：task_state checkpoint、service reconnect CP2、cancel waiting CP3A 与 provider timeout CP3B 已闭环 / 完整异步 LongRunner streaming 未闭环。
...
**当前证据**：`socket.ts` 对 user_message / author_action 使用 `LLM_TURN_TIMEOUT_MS=300000`，与 provider 默认长等待窗口对齐；`handleSend` catch 显示“发送失败，请重试。”；`au10-workbench-recovery-disconnect-timeout` CP1 已证明不可达 provider 失败后不无限 loading，且恢复 provider 后可继续；`au10-workbench-recovery-reconnect` CP2 已证明服务断开/重启后的离线与重连恢复；`au10-workbench-recovery-cancel-waiting` CP3A 已证明确认等待可由真实页面点击拒绝关闭，且不留下 loading、工具调用或 production write；`au10-workbench-recovery-provider-timeout` CP3B 已证明真实 provider receive timeout 后不留下 loading 或 production write，并可恢复到下一轮。仍无完整异步 LongRunner UI 自动化。

**当前状态**：provider failure recovery CP1、service reconnect CP2、cancel waiting CP3A 与 provider timeout CP3B 已闭环 / 完整 LongRunner 恢复未闭环。
...
| SC-AU10-B1 | 发送消息 + loading | 最小闭环 / provider failure CP1、cancel waiting CP3A 与 provider timeout CP3B 已闭环 | AU-01 deterministic 与 LMStudio Tauri 两轮普通聊天；`au10-workbench-matrix-layout-tauri` 覆盖普通消息完成；`au10-workbench-recovery-disconnect-timeout-tauri` 覆盖不可达 provider 后 no-write fallback、loading 清除、恢复后下一轮完成；`au10-workbench-recovery-cancel-waiting-tauri` 覆盖确认等待取消后 loading 清除与下一轮完成；`au10-workbench-recovery-provider-timeout-tauri` 覆盖真实 provider timeout 后 no-write fallback、loading 清除与下一轮完成；缺 LongRunner 恢复态 |
...
| SC-AU10-D2 | task_state 实时显示 | checkpoint closed / 完整异步长任务待补 | `au10-workbench-recovery-taskstate-tauri` 覆盖真实导出动作 RUNNING/CHECKPOINT/COMPLETED UI；`workspace_channel_task_state_test.exs` 覆盖 FAILED；缺完整异步 LongRunner |
| SC-AU10-D3 | 超时/取消等待 | provider failure CP1 + reconnect CP2 + cancel waiting CP3A + provider timeout CP3B 已闭环 / LongRunner 待补 | timeout/catch 有；`au10-workbench-recovery-disconnect-timeout-tauri` 覆盖 provider failure recovery；`au10-workbench-recovery-reconnect-tauri` 覆盖 service reconnect recovery；`au10-workbench-recovery-cancel-waiting-tauri` 覆盖取消等待 no-write 关闭；`au10-workbench-recovery-provider-timeout-tauri` 覆盖真实 provider timeout；缺完整 LongRunner Tauri 证据 |
...
| SC-AU10-F1 | Playwright/Tauri UI 验收 | baseline + task_state + provider failure CP1 + reconnect CP2 + cancel waiting CP3A + provider timeout CP3B 已闭环 / LongRunner 待补 | AU-01/AU-02/AU-04/AU-05/AU-07/AU-08/AU-09/AU-10/VS-10；`au10-workbench-matrix-layout-tauri` 是首条 AU-10 baseline matrix；`au10-workbench-recovery-taskstate-tauri` 覆盖真实导出 task_state checkpoint；`au10-workbench-recovery-disconnect-timeout-tauri` 覆盖 provider failure recovery；`au10-workbench-recovery-reconnect-tauri` 覆盖 service reconnect；`au10-workbench-recovery-cancel-waiting-tauri` 覆盖取消等待；`au10-workbench-recovery-provider-timeout-tauri` 覆盖真实 provider timeout；缺完整 LongRunner 恢复矩阵 |
...
| AU10-GAP-07 — 真实入口 task_state 只到 store | **checkpoint closed（2026-06-17）**：真实“导出全书”动作已通过外部 Tauri driver 证明 RUNNING/CHECKPOINT/COMPLETED UI 可见，FAILED 由 Channel 回归覆盖；取消等待 CP3A 已另由 `au10-workbench-recovery-cancel-waiting` 覆盖，provider timeout CP3B 已另由 `au10-workbench-recovery-provider-timeout` 覆盖；剩余是完整异步 LongRunner streaming | 补验收/补集成 | P0 |
...
|---|---|---|
| `WorkspaceChat` | 当前真实首屏，已承载 provider health、消息、候选、action、adoption、why、reading、archive/memory 的最小证据；baseline layout/state、真实导出 task_state checkpoint、provider failure recovery CP1、service reconnect CP2、cancel waiting CP3A 和 provider timeout CP3B 已被 AU-10 专属 driver 覆盖 | 仍是大型组件，完整异步 LongRunner 和全 action/card 状态矩阵未覆盖 |
| `历史旁路工作台` / `历史旁路 socket helper` | 历史 v3 旁路 UI/helper | 已退役删除，不再作为当前验收基础设施 |
| `UICards` | 卡片组件齐全，业务 action 不再从 card 自行提交 | 缺全 card 视觉/降级验收 |
| `workspace_channel_v3_test.exs` / `workspace_channel_task_state_test.exs` | 后端 action 安全和 task_state 广播局部证据 | 覆盖后端 FAILED 分支，但不证明真实 UI 断线/LongRunner 恢复 |
...
bash scripts/tauri_slice_verify.sh au10-workbench-recovery-provider-timeout
...
后续真正完整闭环前还需要新增：
- AU-10 recovery CP3C Tauri spec：完整异步 LongRunner streaming、恢复后继续操作，以及真实批量生成/推演消费者；
