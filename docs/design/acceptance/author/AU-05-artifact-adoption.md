...
| 契约 / 代码 | 用途 |
|---|---|
| `docs/design/contracts/VS-04-adoption-boundary-contract-pack.md` | CandidateSet、AuthorActionInput choose_candidate、AdoptionDecision、ProjectionHint 规则 |
...
| 当前证据 | `artifacts/slice-verify/au05-conflict-cross-work-recovery-tauri/summary.json` 证明真实 Tauri 工作台在当前作品 transcript 中恢复其它作品来源候选，作者点击“采用这个方向”后，服务端授权 `choose_candidate` 经 `AdoptionBoundary` 返回 `fail_with_recovery`，UI 显示“候选方向采用失败”，`candidate_adopted=false` 且 `production_write_performed=false`；`action_roundtrip_test.exs`、`workspace_channel_v3_test.exs` 覆盖回归 |
| 当前状态 | cross-work candidate recovery failure checkpoint 已闭环 |
| 当前缺口 | 完整 work-scoped adoption store、state trace/projection 归属和跨作品草稿待处理箱仍未闭环 |
| 优先级 | P0 |
...
| SC-AU05-C3 | stale 草稿拒绝 | checkpoint closed | 是，恢复出的 stale candidate 已由真实工作台拒绝；完整 context version / state snapshot freshness 仍未闭环 |
| SC-AU05-C4 | 冲突采纳恢复 | 已设计 | 否 |
| SC-AU05-C5 | 跨作品采纳隔离 | checkpoint closed | 是，恢复出的 cross-work candidate 已由真实工作台 fail_with_recovery；完整 work-scoped adoption store 仍未闭环 |
...
4. stale/conflict/cross-work：已证明恢复出的 stale candidate 和 cross-work candidate 不能写入当前作品；还需补冲突草稿不能静默覆盖当前 canon。
