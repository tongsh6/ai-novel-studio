...
| `ReplayService` / `TraceReplayService` | replay 不重新调用 provider；旧 turn 可按 work/session/turn 查询持久 trace | E2E reply-only replay 有测试；`au07-trace-why-entry` 已回填当前 turn author-safe why/no-provider/no-write 真实 Tauri cross evidence；`au07-persisted-trace-query` 已回填普通旧 turn scoped query / no-provider replay 子证据；`au07-trace-query-scope-negative-matrix` 已回填跨 work/session/turn 负向查询不泄露 trace 子证据 |
...
**当前证据**：`dialogue_gateway_test.exs` DecisionTrace assertions；`v3_full_chain_test.exs` “replay from reply-only trace never calls provider”；`quality_accept au07-trace-why-entry --surface tauri` 从真实 Tauri 工作台完成一轮普通聊天后点击“为什么”，证明当前 turn 有 author-safe trace summary、raw prompt/provider/debug/trace id 不可见，且 why/replay 解释不调 provider、不写状态；`quality_accept au07-persisted-trace-query --surface tauri` 证明 reload 后旧 turn “为什么”会按 work/session/turn scope 查询持久 trace 并渲染 no-provider replay；`quality_accept au07-trace-query-scope-negative-matrix --surface tauri` 证明跨 work/session/turn 负向 replay 查询返回 404 且不泄露 trace；`quality_accept au07-gate-reason-why --surface tauri --provider lmstudio` 证明降级 gate 也可在 why 面板显示作者安全中文解释。

**当前状态**：后端已测试；当前 turn why/no-provider、普通旧 turn 最小 scoped query、partial replay 和 scoped negative matrix 子路径已有 AU-07 真实 Tauri cross evidence；完整 replay、developer view、多类型 UI 仍属于 AU-07 P1，AU-01 不重复造入口。
...
| SC-AU01-D1 普通聊天可追溯且 replay 不调 LLM | trace 有 frame/order；replay 不重新调用 provider | DecisionTrace / ReplayService / TraceReplayService | `DecisionTrace`、`ReplayService`、`TraceReplayService`、why/replay UI | `dialogue_gateway_test.exs`、`v3_full_chain_test.exs`、AU-07 scoped query tests | `au07-trace-why-entry-tauri/summary.json` 作为当前 turn author-safe why/no-provider/no-write cross evidence；`au07-persisted-trace-query-tauri/summary.json` 作为普通旧 turn scoped query / no-provider replay cross evidence；`au07-partial-replay-ui-tauri/summary.json` 作为 partial replay 诚实提示 cross evidence；`au07-trace-query-scope-negative-matrix-tauri/summary.json` 作为 scoped negative matrix cross evidence；`au07-gate-reason-why-tauri-lmstudio/summary.json` 作为降级 gate why 中文解释 cross evidence | 部分实现 | 完整 replay、developer view、多类型 UI 仍归 AU-07 | cross-reference | P1 | `AU07-trace-replay-chat-readonly` |
...
**覆盖结论：13 个用户场景；11/13 已有完整真实页面外部自动化证据（其中 SC-AU01-E1 的证据 owner 为 AU-10 recovery）；SC-AU01-D1 已有 AU-07 当前 turn why/no-provider、普通旧 turn scoped query、partial replay 和 scoped negative matrix cross evidence，但未达完整 replay、developer view、多类型 UI 验收；SC-AU01-C3 已测试但缺真实 UI 反证。当前新增 `au01-turnresult-recorder-ui-consistency` 后已关闭 SC-AU01-B4 的 P1 缺口；AU-01 文件级退出标准已满足，可以进入 AU-02。剩余 P1 为 D1 完整 trace/replay UI cross-reference（owner：AU-07），P2 为 C3 自由讨论 no-slot-form UI 反证。**
...
| AU01-GAP-06 — 普通聊天 recorder / replay / UI 一致性缺文件级证据 | B4 已完成同一 turn 的 UI / TurnResult / recorder / reload 恢复对账；D1 当前 turn why/no-provider、普通旧 turn scoped query、partial replay 和 scoped negative matrix 子路径已由 AU-07 回填，但完整 replay、developer view、多类型 UI 仍缺 | B4 已关闭：`au01-turnresult-recorder-ui-consistency` 证明同一 `turn_id` 的可见 assistant 文本、websocket TurnResult、transcript row 与 reload 恢复 UI 一致；D1 当前 turn author-safe why/no-provider/no-write 已由 `au07-trace-why-entry` 证明，普通旧 turn scoped query / no-provider replay 已由 `au07-persisted-trace-query` 证明，partial replay 诚实提示已由 `au07-partial-replay-ui` 证明，跨 work/session/turn 负向查询不泄露 trace 已由 `au07-trace-query-scope-negative-matrix` 证明，完整 replay UI 继续由 AU-07 owner 补齐，不在 AU-01 中重复造入口 |
...
| `scripts/quality_accept.sh au07-trace-why-entry --surface tauri` | 原生 Tauri 输入框发送普通聊天并从真实消息流点击“为什么”，证明当前 turn author-safe why dialog、trace summary、raw prompt/provider/debug 不可见，且 why/replay 解释不调 provider、不写状态 | 旧 turn trace query 由 `au07-persisted-trace-query` 单独证明；本命令不证明完整 ReplayReport 产品入口，剩余 owner 仍为 AU-07 |
| `scripts/quality_accept.sh au07-persisted-trace-query --surface tauri` | 原生 Tauri 输入框发送普通聊天，reload 后从旧 turn “为什么”入口触发 work/session/turn scoped persisted replay query，证明普通旧 turn no-provider replay 可见 | partial replay 作者提示由 `au07-partial-replay-ui` 单独证明；仍不证明 developer view 或多类型 ReplayReport 产品入口 |
| `scripts/quality_accept.sh au07-partial-replay-ui --surface tauri` | 原生 Tauri reload 后从旧 turn “为什么”入口呈现缺 refs 的 persisted partial replay，证明 `result_status=partial`、missing refs、no-provider/no-write 与 author-safe warning 可见 | 不证明 developer view 或多类型 ReplayReport 产品入口；剩余 owner 仍为 AU-07 |
| `scripts/quality_accept.sh au07-trace-query-scope-negative-matrix --surface tauri` | 原生 Tauri 建立普通 turn 并 reload 恢复旧消息后，从外部请求 replay API，证明原 work/session/turn 返回 200，跨 work、跨 session、same-work other session 和 missing turn 均 404 且不泄露 trace | 不证明 developer view、旧 workspace_id 迁移或多类型 ReplayReport 产品入口；剩余 owner 仍为 AU-07 |
| `scripts/quality_accept.sh au07-gate-reason-why --surface tauri --provider lmstudio` | 原生 Tauri 档案入口触发 real LM Studio 多步 MicroPlan 降级后，从消息流“为什么”入口展示 action_scope 中文 gate/reason、no-provider replay 和 no-write 证据 | 不证明完整 reason catalog、developer view 或多类型 ReplayReport 产品入口；剩余 owner 仍为 AU-07 |
