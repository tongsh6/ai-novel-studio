...
| SU02-GAP-03 — 跨作品消息/上下文隔离缺验收 | 已补齐：消息流不串、AU09 已证明记忆/档案/why 不串，`su02-artifact-projection-trace-isolation` 已证明 artifact/projection/trace 不串作品，`au07-trace-query-scope-negative-matrix` 已证明 replay API 跨 work/session 负向查询不泄露 trace | 保持回归；角色/统计扩展矩阵作为 P2 |
