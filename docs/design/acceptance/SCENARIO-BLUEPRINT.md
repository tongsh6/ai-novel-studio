...
| P0 | 确认幂等与重审闭环 | 高风险计划 → 确认 → 重新 gate → 工具执行；重复点击不重复执行；旧确认不能跨当前 turn/作品/历史会话执行；确认前上下文变化必须被重新消费；disabled action 不得被提交；确认后工具失败不得半写入 | AU-04/AU-06 | 主路径、重复确认、当前-turn stale、expired confirmation、disabled confirmation、history-readonly confirmation、cross-work confirmation、latest-context rebase 和工具失败恢复已补；继续补持久 snapshot / replay / LLM timeout / retry action 矩阵 | `au04-confirm-before-execute` 已关闭高风险确认主路径；`au04-confirm-idempotency-ui` 已证明同一确认动作快速重复点击只执行一次；`au04-stale-confirmation-ui` 已证明 follow-up 推进当前 turn 后旧确认被 stale 拒绝；`au04-confirmation-ttl-ui` 已证明 expired confirmation 被拒绝；`au04-disabled-confirmation-action-ui` 已证明 disabled confirm 可见但不可提交；`au04-history-confirmation-readonly` 已证明历史只读 confirmation 不可执行；`au04-cross-work-confirmation-guard` 已证明跨作品切换后源 confirmation 不泄漏不执行；`au04-latest-context-rebase-confirmation` 已证明确认前真实改名后 binding/trace 消费最新 Work revision/title；`au04-confirmation-tool-failure-recovery` 已证明确认后工具失败 no-draft/no-write；执行权仍是 v3 核心安全边界，下一步补持久 snapshot、replay、LLM timeout 和 retry action |
