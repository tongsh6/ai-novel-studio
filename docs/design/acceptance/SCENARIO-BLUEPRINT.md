...
|---|---|---|---|---|
| 系统启动与模型配置 | 本地启动后知道 AI 是否可用，可配置/测试供应商 | SU-01 | 7/10 最小真实 Tauri 证据；B2/C3 已补 secret redaction checkpoint，B3 已补非法 endpoint 校验与模型列表成功矩阵 checkpoint；3/10 仍缺完整真实桌面矩阵 | DeepSeek、运行时切换 API、供应商实时模型列表、工作台模型设置 UI、Tauri 偏好/Keychain、provider/model 审计日志已实现；`su01-model-provider-switching` 已验收 Stub 切换主路径，`su01-lmstudio-disconnected-health` 已验收 LM Studio 未启动断开态，`su01-provider-endpoint-validation` 已验收非法 endpoint 阻断，`su01-provider-model-list-success` 已验收 DeepSeek/Anthropic/LM Studio 模型列表成功管线，`su01-api-key-secret-redaction` 已验收 API Key 配置流脱敏边界，仍缺 Keychain WebView 端到端、live vendor/云端本地失败矩阵和跨平台 secret 策略 |
| 作品空间管理 | 命名新增、切换、重命名、删除或移出作品，保证上下文隔离和恢复策略正确 | SU-02 | 10/13 已有最小真实 Tauri 前端闭环；3/13 部分 | 运行时作品菜单、快速未命名创建、命名新增、改名、安全废弃/移出、leave/join、消息流隔离和 reload 恢复已有最小证据；真实慢 LLM 迟到结果归属和完整 artifact/projection/trace 隔离矩阵仍缺 |
| AI 身份与显示 | 给 AI 起名，且按作品隔离 | SU-03 | 6/6 真实 Tauri 验收；SC-SU03-C2 有 LM Studio payload 证据 | 已补 work-scoped UI 显示名、设置/重置、按作品隔离和 provider/prompt/TurnResult 边界；后续只需随工作台重构做回归 |
| 自然创作对话 | 作者能持续自然讨论，不被表单化打断 | AU-01/AU-02 | AU-01: 7/13 最小真实 Tauri checkpoint；AU-02: 7/12 最小真实 Tauri checkpoint | 普通聊天两轮、空消息 guard、候选继续探索、候选卡后自由追问、候选 no-MicroPlan 和候选采纳桥接已有 Tauri 证据；仍缺 AU-01 乱码/frame validation/replay UI 异常矩阵、AU-02 多轮追问和真实 LLM 质量复验 |
...
|---:|---|---|---|---|---|
| P0 | 作品生命周期管理闭环 | 命名新增作品、修改作品名、安全删除或移出作品、删除当前作品后切换、lastOpened 不恢复已移出作品 | SU-02 | 已有最小闭环/继续补矩阵 | `su02-work-lifecycle-management` 已证明真实工作台命名新增、改名、安全移出和 fallback；`su02-work-restart-recovery` 已证明 reload 恢复 existing lastOpened，且 stale/discarded lastOpened 回退到真实 Work；仍需补恢复/归档管理入口和异常矩阵 |
| P0 | 作品切换隔离闭环 | 创建两个作品、切换、发送消息、pending 返回不串作品、重启恢复 | SU-02/AU-03 | 已有最小闭环/继续补矩阵 | `su02-work-switching` 已证明真实菜单快速创建/切换、leave/join 和消息流不串；`su02-work-restart-recovery` 已证明重启恢复最小闭环；仍需补真实慢 LLM 迟到归属和 artifact/projection/trace 全矩阵 |
| P0 | 普通聊天真实工作台闭环 | 打开工作台、输入创作聊天、收到自然回复、不出现执行卡、可继续下一轮 | AU-01/E2E | 已闭环/继续补异常矩阵 | `au01-ordinary-chat-two-turn-roundtrip` 已证明真实 Tauri 两轮普通聊天：user/assistant 顺序可见、thinking 清退、`generate_micro_plan=false`、无 MicroPlan、无 action/candidate/adoption UI；`--real-lmstudio` 证明真实 provider 两轮 form_frame 请求。`au01-empty-message-guard` 已证明空白输入不创建 turn 且后续聊天可恢复。剩余是乱码、frame validation、replay UI 和异常恢复矩阵 |
...
| P0 | 确认幂等与重审闭环 | 高风险计划 → 确认 → 重新 gate → 工具执行；重复点击不重复执行 | AU-04/AU-06 | 主路径已补/继续补重复点击与 stale 矩阵 | `au04-confirm-before-execute` 已关闭高风险确认主路径；执行权仍是 v3 核心安全边界，下一步补同一 `idempotency_key` 重复点击、TTL、stale/cross-work 和完整 ConfirmationBinding |
| P0 | 候选方向操作闭环 | 模糊输入 → 候选方向 → 可点选继续探索或直接自由追问 → 明确采纳时进入 adoption boundary | AU-02/AU-05/AU-10 | 已闭环 | `au02-candidate-continuation`、`au02-freeform-followup-after-candidate` 与 `au02-candidate-adoption-bridge` 已提供真实 Tauri 证据；高风险候选 confirmation checkpoint 见 `au05-adoption-safety-freshness`，stale restored candidate rejection checkpoint 见 `au05-stale-conflict-cross-work-freshness`，cross-work recovery checkpoint 见 `au05-conflict-cross-work-recovery`，canon conflict recovery checkpoint 见 `au05-canon-conflict-recovery` |
...
| P0 | 作品内会话管理闭环 | 一个作品下 N 个会话；列表/搜索/归档；历史会话只读；最新作品背景仍生效 | AU-03/SU-02/AU-07 | 新建 active、历史只读与从历史继续 checkpoint 已补 / 继续补集成 | `au03-session-new-active` 已证明真实工作台新建空白 active session、旧 active 退出并只读可回看、下一轮消息绑定新 session 且不带旧 transcript；`au03-session-history-readonly` 已证明 exited 历史会话只读回看和返回 active session；`au03-branch-from-history` 已证明历史会话“从这里继续”创建带 source refs 的新 active branch；仍需补搜索命中定位、归档过滤当前入口复跑和最新作品背景 SSOT |
...
| 发现 | 影响 | 建议 |
|---|---|---|
| SU-01 已按 2026-06-19 checkout 重算为 10 个场景 | Health、运行时切换、LM Studio 断开态都有最小真实 Tauri 证据；B2/C3 已补 secret redaction checkpoint，B3 已补非法 endpoint 校验与模型列表成功矩阵 checkpoint，但还不能误判为完整供应商配置完成 | 下一步补 Keychain WebView 端到端、live vendor/云端本地完整异常矩阵和跨平台 secret 策略 |
| `acceptance/README.md` 的覆盖率和测试计数已滞后 | 新会话会误判完成度 | 下一步先更新 README 总览，改为引用台账和本蓝图 |
| SU-02 已按 2026-06-19 checkout 重算为 13 个场景 | 运行时切换、作品生命周期和重启恢复最小闭环已补，不应继续误判为“没有切换 UI”“不能命名/改名/移出作品”或“lastOpened 仍只靠 localStorage” | 下一步继续补真实慢 LLM 迟到结果、恢复/归档管理入口和完整 artifact/projection/trace 隔离矩阵 |
| AU-01 普通聊天主路径与空消息 guard 已按当前 checkout 重算 | `au01-ordinary-chat-two-turn-roundtrip` 已从真实 Tauri 工作台证明两轮自然聊天、可见消息顺序、thinking 清退、no-MicroPlan、无 action/candidate/adoption UI，并有 real LMStudio provider 请求证据；`au01-empty-message-guard` 已证明空白输入不会发送 `user_message` 或追加 DOM 消息，且后续普通聊天可恢复；不应继续误判为“缺真实工作台 walkthrough”或“空消息无真实页面证据” | 后续 AU-01 优先补乱码 JSON、frame validation 友好错误、provider 异常矩阵和 replay/trace UI，而不是重复补普通聊天主路径 |
...
| AU-02 候选方向状态已重算 | backend fallback/real_llm 证据已进入验收文档；候选卡点选 continuation、候选卡后自由手输追问、采纳桥接、高风险候选 confirmation checkpoint、stale restored candidate rejection checkpoint、cross-work recovery checkpoint 和 canon conflict recovery checkpoint 已闭环；continuation 明确是 `user_message.candidate_selection`，自由追问明确是不带 `candidate_selection` 的普通 `user_message`，都不是 `author_action.choose_candidate` | 下一步补多轮/真实 LLM 质量与未采纳候选不入事实反证 |
| AU-06 与 AU-04 都指向确认 lifecycle 缺口 | 重复但合理，说明它是跨文档主风险 | 建议合并为一个 P0 场景族追踪 |
| AU-04 已按真实工作台入口重算 | `WorkspaceChat` 当前通过服务器 `available_actions` 提交 `author_action`；`au04-confirm-before-execute` 已证明高风险确认主链，`ActionValidator` 已补 `behavior_ref` / `target_ref` / `candidate_*` / `idempotency_key` 与服务端 action 的精确绑定；确认、候选和采纳相关动作已有多条真实 Tauri checkpoint | 优先补 action_result 全状态、重复点击、stale/idempotency/disabled UI、TTL、rebased snapshot 和 AU-10 恢复态，而不是重复接旧 helper |
...
| 2 | 重写 SU-02 作品切换与生命周期验收 | 场景化 case + 当前证据 + P0 缺口 | 已完成运行时切换对账；命名新增、改名、删除或移出作品、重启恢复已补最小真实 Tauri checkpoint，后续补恢复/归档入口和完整隔离矩阵 |
| 3 | 更新 AU-02 探索验收 | 候选 fallback、real_llm、候选点选/freeform follow-up/采纳桥接场景 | 已完成首轮对账；候选卡后自由追问 checkpoint 已补，后续进入 AU-03 |
