...
| 执行与确认 | AI 可提计划，系统负责门禁、确认和重审 | AU-04/AU-06 | AU-04: 7/18 最小真实 Tauri checkpoint，6/18 有局部测试；AU-06: 0/17 完整真实前后端验收 | `au04-confirm-before-execute` 已证明高风险执行先确认、确认前 no-tool/no-write、真实 `author_action`、重新 gate 和 pending artifact；`au04-confirm-idempotency-ui` 已证明重复确认不重复执行；`au04-stale-confirmation-ui` 已证明当前 turn 推进后旧确认被 stale 拒绝且 no-tool/no-draft；ConfirmationBinding re-gate refs 已有局部强契约；取消等待主路径也有 AU-10 checkpoint。disabled、TTL、跨作品/历史会话、持久 snapshot、behavior history/replay 和失败恢复仍不足 |
...
| P0 | 确认幂等与重审闭环 | 高风险计划 → 确认 → 重新 gate → 工具执行；重复点击不重复执行；旧确认不能跨当前 turn 执行 | AU-04/AU-06 | 主路径、重复确认和当前-turn stale 已补；继续补 TTL / cross-work / history 矩阵 | `au04-confirm-before-execute` 已关闭高风险确认主路径；`au04-confirm-idempotency-ui` 已证明同一确认动作快速重复点击只执行一次；`au04-stale-confirmation-ui` 已证明 follow-up 推进当前 turn 后旧确认被 stale 拒绝；`ConfirmationBinding` 已强制 snapshot/gate proof；执行权仍是 v3 核心安全边界，下一步补 TTL、cross-work、持久 snapshot 和 replay |
...
| AU-04 已按真实工作台入口重算 | `WorkspaceChat` 当前通过服务器 `available_actions` 提交 `author_action`；`au04-confirm-before-execute` 已证明高风险确认主链，`au04-confirm-idempotency-ui` 已证明重复确认不重复执行，`au04-stale-confirmation-ui` 已证明当前 turn 推进后旧确认被 stale 拒绝且 no-tool/no-draft，`ActionValidator` 已补 `behavior_ref` / `target_ref` / `candidate_*` / `idempotency_key` 与服务端 action 的精确绑定；`ConfirmationBinding` 已强制 `rebased_state_snapshot_ref` / `gate_result_refs` 并在 confirmation ack / reason_codes 留下 proof；确认、候选和采纳相关动作已有多条真实 Tauri checkpoint | 优先补 action_result 全状态、disabled UI、TTL、跨作品/历史只读、持久 snapshot、replay 和 AU-10 恢复态，而不是重复接旧 helper |
