...
| 自然创作对话 | 作者能持续自然讨论，不被表单化打断 | AU-01/AU-02 | AU-01: 6/13 最小真实 Tauri checkpoint；AU-02: 6/12 最小真实 Tauri checkpoint | 普通聊天两轮真实工作台闭环、候选继续探索、候选 no-MicroPlan 和候选采纳桥接已有 Tauri 证据；仍缺 AU-01 异常矩阵、AU-02 直接手输追问反证、多轮追问和真实 LLM 质量复验 |
...
|---:|---|---|---|---|---|
| P0 | 作品生命周期管理闭环 | 命名新增作品、修改作品名、安全删除或移出作品、删除当前作品后切换、lastOpened 不恢复已移出作品 | SU-02 | 已有最小闭环/继续补矩阵 | `su02-work-lifecycle-management` 已证明真实工作台命名新增、改名、安全移出和 fallback；仍需补重启后 lastOpened 不恢复已移出作品、恢复/归档管理入口和异常矩阵 |
| P0 | 作品切换隔离闭环 | 创建两个作品、切换、发送消息、pending 返回不串作品、重启恢复 | SU-02/AU-03 | 已有最小闭环/继续补矩阵 | `su02-work-switching` 已证明真实菜单快速创建/切换、leave/join 和消息流不串；仍需补真实慢 LLM 迟到归属、artifact/projection/trace 全矩阵和重启恢复 |
| P0 | 普通聊天真实工作台闭环 | 打开工作台、输入创作聊天、收到自然回复、不出现执行卡、可继续下一轮 | AU-01/E2E | 已闭环/继续补异常矩阵 | `au01-ordinary-chat-two-turn-roundtrip` 已证明真实 Tauri 两轮普通聊天：user/assistant 顺序可见、thinking 清退、`generate_micro_plan=false`、无 MicroPlan、无 action/candidate/adoption UI；`--real-lmstudio` 证明真实 provider 两轮 form_frame 请求。剩余是空消息、乱码、frame validation、replay UI 和异常恢复矩阵 |
...
| P0 | 作品内会话管理闭环 | 一个作品下 N 个会话；列表/搜索/归档；历史会话只读；最新作品背景仍生效 | AU-03/SU-02/AU-07 | 新建 active 与历史只读 checkpoint 已补 / 继续补集成 | `au03-session-new-active` 已证明真实工作台新建空白 active session、旧 active 退出并只读可回看、下一轮消息绑定新 session 且不带旧 transcript；`au03-session-history-readonly` 已证明 exited 历史会话只读回看和返回 active session；仍需补从历史分支继续 Tauri 验收、搜索命中定位、归档过滤当前入口复跑和最新作品背景 SSOT |
...
| 发现 | 影响 | 建议 |
|---|---|---|
| SU-01 已按 2026-06-19 checkout 重算为 10 个场景 | Health、运行时切换、LM Studio 断开态都有最小真实 Tauri 证据；B2/C3 已补 secret redaction checkpoint，B3 已补非法 endpoint 校验 checkpoint，但还不能误判为完整供应商配置完成 | 下一步补 Keychain WebView 端到端、真实 provider 模型列表成功矩阵、DeepSeek/Anthropic/LM Studio 完整异常矩阵和跨平台 secret 策略 |
| `acceptance/README.md` 的覆盖率和测试计数已滞后 | 新会话会误判完成度 | 下一步先更新 README 总览，改为引用台账和本蓝图 |
| SU-02 已按 2026-06-19 checkout 重算为 13 个场景 | 运行时切换和作品生命周期最小闭环已补，不应继续误判为“没有切换 UI”或“不能命名/改名/移出作品” | 下一步继续补 pending 迟到结果、重启恢复、恢复/归档管理入口和完整隔离矩阵 |
| AU-01 普通聊天主路径已按当前 checkout 重算 | `au01-ordinary-chat-two-turn-roundtrip` 已从真实 Tauri 工作台证明两轮自然聊天、可见消息顺序、thinking 清退、no-MicroPlan、无 action/candidate/adoption UI，并有 real LMStudio provider 请求证据；不应继续误判为“缺真实工作台 walkthrough” | 后续 AU-01 优先补空消息、乱码 JSON、frame validation 友好错误、provider 异常矩阵和 replay/trace UI，而不是重复补普通聊天主路径 |
...
| AU-02 候选方向状态已重算 | backend fallback/real_llm 证据已进入验收文档；候选卡点选 continuation、采纳桥接、高风险候选 confirmation checkpoint、stale restored candidate rejection checkpoint、cross-work recovery checkpoint 和 canon conflict recovery checkpoint 已闭环；continuation 明确是 `user_message.candidate_selection`，不是 `author_action.choose_candidate` | 下一步转向 P1 长篇产出主链：章节计划最小闭环 |
| AU-06 与 AU-04 都指向确认 lifecycle 缺口 | 重复但合理，说明它是跨文档主风险 | 建议合并为一个 P0 场景族追踪 |
| AU-04 已按真实工作台入口重算 | `WorkspaceChat` 当前通过服务器 `available_actions` 提交 `author_action`；确认、候选和采纳相关动作已有多条真实 Tauri checkpoint；`ActionValidator` 已补 `behavior_ref` / `target_ref` / `candidate_*` / `idempotency_key` 与服务端 action 的精确绑定 | 优先补 action_result 全状态、stale/idempotency/disabled UI、TTL、rebased snapshot 和 AU-10 恢复态，而不是重复接旧 helper |
...
| 2 | 重写 SU-02 作品切换与生命周期验收 | 场景化 case + 当前证据 + P0 缺口 | 已完成运行时切换对账；命名新增、改名、删除或移出作品已补最小真实 Tauri checkpoint，后续补恢复/归档和完整隔离矩阵 |
...
| SC-SU02-B5 | 删除或移出作品 | 已补：对当前作品执行二次确认后的安全废弃/移出；删除当前作品后切到另一个真实 Work 或新建未命名 Work；重启恢复矩阵仍待补 |
