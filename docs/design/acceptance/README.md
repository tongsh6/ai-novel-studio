...
>
> 重要：本目录的早期 AU/SU 覆盖率是“文档内场景覆盖率”，不等同于完整产品验收。当前验收恢复入口以 [场景化验收蓝图](SCENARIO-BLUEPRINT.md)、本 README 的状态表、`quality/acceptance/scenarios.yml` 和 `docs/project-ledger.md` 的最新补充共同对账；不要只凭旧单测数量或历史 artifact 判定场景已验收。
...
| 文档 | 作用 | 当前状态 |
|------|------|----------|
| [SCENARIO-BLUEPRINT](SCENARIO-BLUEPRINT.md) | 从完整功能蓝图出发，识别跨 AU/SU 的主流程场景和缺口 | 当前覆盖主线与二轮收口顺序；SU-01 → SU-03、AU-01 → AU-12、E2E-01 已按 2026-06-22 口径完成二轮复核，剩余 P1/P2 按文件级矩阵和 quality manifest 滚动收敛 |
| [E2E-01](e2e/E2E-01-full-chain.md) | 用真实 LLM / SQLite / Channel 证明主链端到端路径 | 文件级可交付 / 三轮 security checkpoint 已复核：12/13 已验收；1/13 已测试；0/13 部分实现；P0/P1/P2 已关闭，2026-06-22 已复跑 E2E 聚合 runner、E4 downgrade、E6 read-only tool trace、E9 ReplayReport 六问真实页面 checkpoint，并新增 E8/E13 `e2e-01-channel-action-security` 外部协议 fuzz；E10 为 AU-07 cross-reference |
...
| 文档 | 能力 | 场景 | 覆盖率 | 状态 |
|------|------|------|--------|------|
| [AU-01](author/AU-01-chat.md) | 与 AI 聊创作 | 13 | 11/13 有完整真实页面外部自动化证据；D1 有 AU-07 当前 turn、普通旧 turn scoped query、partial replay、scope negative matrix 与 gate reason why cross evidence；C3 已测试；文件级可进入 AU-02 | 普通聊天两轮真实工作台闭环已补，含可见 user/assistant 顺序、thinking 清退、`generate_micro_plan=false`、无 MicroPlan、无 action/candidate/adoption UI；`--real-lmstudio` 证明真实 provider 两轮 form_frame 请求。空消息 guard、provider recovery、乱码 JSON 友好降级、frame validation 友好错误和 TurnResult/recorder/UI 同源对账已补真实页面验收；`au07-trace-why-entry` 已回填 D1 当前 turn author-safe why/no-provider/no-write 子证据，`au07-persisted-trace-query` 已回填普通旧 turn scoped query / no-provider replay 子证据，`au07-partial-replay-ui` 已回填 partial replay 诚实提示子证据，`au07-trace-query-scope-negative-matrix` 已回填 scoped negative matrix 子证据，`au07-gate-reason-why` 已回填降级 gate why 中文解释子证据；剩余 P1 为完整 replay、developer view、多类型 UI cross-reference（owner：AU-07），P2 为 no-slot-form UI 反证 |
...
| [AU-03](author/AU-03-context.md) | AI 了解我的作品 | 20 | 文件级可交付：13/20 已验收；2/20 已测试；3/20 部分实现；1/20 已实现未验收；0/20 未实现；1/20 不确定；当前可进入 AU-04 | 新建会话、历史会话只读回看、从历史继续创建新分支会话、归档过滤、最新 Work 背景 SSOT、active transcript 分层、开放 behavior context、上下文来源 UI、长会话压缩和跨作品/记忆分层已有当前可复跑真实 Tauri 证据；`au03-current-work-context-ssot --real-lmstudio` 证明最新 Work 与 active transcript 进入真实 provider prompt；`au11-missing-workstate-policy` 作为 B1 missing WorkState cross evidence，证明缺章节/正文/人物状态时不编造当前章；`au06-single-active-confirmation` 作为 A4 dependency evidence，证明第二个 confirmation turn 消费上一轮开放 behavior 的 author-safe context ref；`AU03-file-level-closure.md` 已补文件级收口记录；剩余 P1/P2 为完整 empty/work-only/failure UI、显式 archived source、AU-07 replay/trace、搜索命中定位和压缩策略细化 |
...
| [AU-07](author/AU-07-trace-and-replay.md) | 决策溯源透明度 | 16 | 文件级可交付：13/16 已验收；0/16 已测试；3/16 部分实现；P0 已关闭，2026-06-22 二轮按依赖关系关闭旧 turn scoped query、partial replay UI、gate reason why 与 scope negative matrix，当前可进入 AU-08 | `au07-trace-why-entry`、`au07-persisted-trace-query`、`au07-partial-replay-ui`、`au07-gate-reason-why`、`au07-trace-query-scope-negative-matrix`、`au07-state-trace-adoption-replay`、`au07-behavior-trace-terminal-replay` 七个 AU-07 本体入口已有真实 Tauri / quality 证据；作者 why dialog author-safe、不重调模型、不写生产状态，reload 后旧 turn “为什么”会按 work/session/turn 查询持久 trace 并渲染 no-provider replay，缺 refs 时显示 partial warning 和 missing refs，`action_scope` 降级可显示中文 gate/reason，跨 work/session/turn 负向 replay 查询返回 404 且不泄露 trace，StateTrace/adoption/projection replay producer 与 Behavior terminal replay producer均有当前真实 Tauri 证据。`e2e-01-readonly-tool-trace` 与 `e2e-01-replay-report` 的默认 Tauri 和 real LM Studio quality 入口也已复跑通过，补上 summary-level ToolTrace refs 持久回查和 ReplayReport 六问 cross evidence；剩余 P1 是 developer 双视图、完整 ToolTrace registry snapshot / redacted I/O、多类型 replay UI 矩阵和 reason catalog 深化，旧 workspace_id 迁移细化登记为 P2 |
| [AU-08](author/AU-08-reading-mode.md) | 阅读我的作品 | 16 | 文件级可交付：12/16 已验收；2/16 已实现未验收；2/16 部分实现；P0 已关闭，2026-06-22 二轮已关闭 cross-reference driver 漂移、D1 no-write 专项和 B4 return-context 专项，当前可进入 AU-09 | 采纳正文、编辑后采纳、未采纳不入阅读、跨作品隔离、多章导航、空章诚实显示、短章 audit、导出、STALE banner、阅读查看/导出/返回 no-write，以及返回工作台后 follow-up 同 work/session 均有当前真实 Tauri / quality acceptance 入口；二轮复跑 5 个 AU-08 本体入口和 3 个 cross evidence 全部通过，修复 `p1-chapter-draft-generation` 旧文案等待，并新增 `au08-reading-readonly-no-write`、`au08-reading-return-context`；剩余 P1 是 projection refresh 专用 action/no-write driver、REBUILDING/FAILED 状态来源和离线降级 |
| [AU-09](author/AU-09-story-memory.md) | 管理故事设定 | 14 | 文件级可交付：12/14 已验收；2/14 部分实现；P0 已关闭，2026-06-22 二轮已关闭管理页可见筛选矩阵 P1 和 archive stats current driver P1，当前可进入 AU-10 | 10 个当前可复跑 AU-09 Tauri 入口已挂入 quality acceptance 并在二轮串行复跑通过：记忆创建确认召回、档案概览 stats/detail current driver、管理入口与生命周期终态排除、管理页可见筛选矩阵、lifecycle/reference author-safe 追溯、伏笔/规则 adoption、角色主档案、有效期窗口、跨作品隔离、AU-03 会话/记忆分层。历史 `au09-archive-real-data` / `au09-memory-recall-context` 仅作背景；普通旧 turn scoped query 与 partial replay UI 已由 AU-07 回填；剩余 P1/P2 是 pending inbox、developer replay、多类型 replay UI、Channel 管理入口和完整独立 MemoryTrace/StateTrace 表；排序/分页/limit UI 深化降为 P2。 |
...
| [AU-12](author/AU-12-work-profile.md) | 查看与核对作品档案 | 11 | 文件级可交付：10/11 已验收；1/11 已测试；P0/P1 已关闭，2026-06-22 二轮已关闭 correction 修订意图与读取失败诚实降级，当前可进入 E2E-01 | `au12-work-profile-overview`、`au12-work-profile-status-isolation`、`au12-correction-intent-roundtrip` 与 `au12-profile-read-failure-degrade` 已挂入真实 Tauri / quality acceptance 并在二轮串行复跑通过，覆盖 works 立项概览、accepted/tentative 状态、空字段、读取失败诚实错误态与重试恢复、概览/大纲/角色/伏笔/规则导航、跨作品隔离、no-write、UUID 脱敏，以及从档案发起立项修订意图回到对话主链并产生 pending `world_setting` 的采纳边界；A2 prompt 同源为实现/测试约束，缺同轮 provider prompt 外部证据；accepted `world_setting` 物化 works 字段为 P2 contract |
...
> AU-12（作品档案）已有文件级真实 Tauri 证据：AU12-I1 的 works/profile 单一事实源显示侧、AU12-I2 只读 no-write、AU12-I3 不泄漏内部 id、AU12-I4 当前作品隔离、AU12-I5 状态可辨、AU12-I6 空字段/读取失败诚实显示、AU12-I7 correction intent 回到对话与采纳边界均已验证；AU12-I1 的同轮 provider prompt proof 和 accepted `world_setting` 物化 works 字段仍为 P2。系统 15 不变量矩阵暂不扩列，等 AU12 accepted artifact / works 字段 contract 补齐后再整体重算。

---

## 测试总览

本节只列局部测试入口和质量入口，不再维护固定“总测试数”。真实场景验收以 `quality/acceptance/scenarios.yml` 和 `scripts/quality_accept.sh` 的当前 manifest 为准；后端/Channel/单元测试只能作为局部证据。
...
2026-06-22 复核：`p1-chapter-plan-minimum` 已从旧 `known-gap/blocked` 口径恢复为 `nightly`；新增补齐 `p1-chapter-overwrite-confirm`、`p1-chapter-expansion`、`p1-chapter-word-count-target`、`p1-plan-incremental`、`vs00c-cp0-missing-chapter-block` 五个已有 Tauri driver 的 quality manifest，并已全部通过真实 Tauri `quality_accept` 复跑。`quality/acceptance/known-gaps.md` 当前无 blocked 场景。

---

## 缺口总览

本表是跨文件缺口归并视图。`优先级` 保留缺口来源的历史/剩余风险等级；若 `类型` 已标为 `closed`、`done` 或 `已闭环`，不得再按未完成 P1/P0 处理。当前是否应在某个文件内继续关闭，以对应验收文件的二轮矩阵、`tasks/slices/*file-level-closure.md` 和最新 quality manifest 为准。
...
| 记忆/行为/来源摘要上下文 | AU-03 AU03-GAP-05~10 | memory recall、author-safe 来源摘要、归档过滤、开放 behavior context 和长会话 checkpoint 已补；显式 archived source、真实故障 UI 和 AU-07 replay/trace 继续补 | P1/P2 |
...
| 工作台 why 入口与中文解释 | AU-07 AU07-GAP-01~02 | 当前 Tauri/quality 入口已恢复并复跑；旧 turn query、partial replay 和 action_scope gate why 已补；完整 reason catalog/developer code 双视图待补 | P1 |
...
| projection refresh no-write 与跨作品隔离 | AU-08 AU08-GAP-05~06 | 跨作品隔离已由 SU-02 证明；阅读模式查看/导出/返回 no-write 已由 `au08-reading-readonly-no-write` 证明；专用 refresh action/no-write driver 待补 | P1 |
| 阅读模式错误态与 UI 自动化 | AU-08 AU08-GAP-07~09 | 核心阅读质量入口已补，`au08-reading-readonly-no-write` 已覆盖查看/导出/返回只读，`au08-reading-return-context` 已覆盖返回后同 work/session follow-up；离线/失败态待补 | P1/P2 |
| 记忆管理 REST/Channel 入口与作品档案真实数据 | AU-09 AU09-GAP-01~03 | 文件级 P0 已关闭：管理入口、档案概览 stats/detail current driver、伏笔/规则 adoption、角色主档案、跨作品档案/记忆隔离均有当前 Tauri / quality 证据；pending inbox、Channel 管理入口和正式设计追溯登记为 P1/P2 | done/P1/P2 |
...
| 记忆引用日志、作者溯源、AU-03 会话分层 | AU-09 AU09-GAP-09~12 | author-safe recall/lifecycle reference log、why 来源、记忆详情追溯和 AU-03 会话/记忆分层已闭环；普通旧 turn scoped query 与 partial replay UI 已由 AU-07 回填；developer replay、多类型 replay UI 和完整独立 trace 表登记为 P1 | done/P1 |
...
| 作品档案后续立项物化与 prompt proof | AU-12 SC-AU12-A2/C2 | works 立项概览、accepted/tentative 状态、空字段、读取失败诚实降级、跨作品隔离、tab 导航、no-write 和 correction 修订意图均已闭环；accepted `world_setting` 物化 works 字段、同轮 provider prompt proof 和立项要素深扩展登记为 P2 | P2 |
...
**总缺口：持续重算中。当前 SU-01~03、AU-01~12、E2E-01 已按场景化口径重算或新立；SU-01 B3 模型列表成功矩阵已由 `su01-provider-model-list-success` 补最小真实工作台 checkpoint，B4 测试连接失败反馈与恢复已由 `su01-provider-test-failure-ui` 补真实 Tauri 证据，当前 local-file secret 写读已由 `su01-local-secret-file-roundtrip` 补真实 Tauri 证据；live vendor/失败矩阵仍是 P1，Windows/Linux local-file 页面矩阵降为 P2 平台回归，不再是 SU-01 external blocker。SU-02 空库启动未命名作品、作品生命周期命名新增/改名/安全移出、慢回复迟到归属、重启恢复和 artifact/projection/trace 隔离已有真实 Tauri checkpoint，P0/P1 已闭环，恢复/归档管理、后端不可用 UX、大列表和异常矩阵为 P2 后续；SU-03 work-scoped AI 显示名、默认回退、作品隔离和 UI-only provider 边界已闭环，当前可进入 AU-01。AU-01 普通聊天两轮、空消息 guard、乱码 JSON recovery、frame validation friendly error 和 TurnResult/recorder/UI 同源对账真实工作台 checkpoint 已补，`au07-trace-why-entry` 已作为 D1 当前 turn author-safe why/no-provider/no-write cross evidence 回填，`au07-persisted-trace-query` 已作为 D1 普通旧 turn scoped query / no-provider replay cross evidence 回填，`au07-partial-replay-ui` 已作为 D1 partial replay 诚实提示 cross evidence 回填，`au07-trace-query-scope-negative-matrix` 已作为 D1 scoped negative matrix cross evidence 回填，文件级可进入 AU-02；剩余完整 replay、developer view、多类型 UI 登记为 AU-07 cross-reference，C3 no-slot-form UI 反证为 P2。AU-02 模糊创意自然探索/no-slot-form、真实 LMStudio 中文质量、坏候选 fallback UI、候选 continuation/multiturn context/freeform follow-up/adoption bridge、未采纳阅读/事实反证和 D2 候选 schema/codegen 回归已闭环，文件级可进入 AU-03。AU-03 新建会话、历史只读、从历史继续、归档过滤、最新 Work 背景 SSOT、active transcript 分层、开放 behavior context、上下文来源、长会话和跨作品/记忆分层已补当前可复跑真实 Tauri 证据，`au03-current-work-context-ssot --real-lmstudio` 已证明真实 provider prompt 分层，`au11-missing-workstate-policy` 已作为 B1 missing WorkState cross evidence 回填，`au06-single-active-confirmation` 已作为 A4 dependency evidence 回填，P0 已关闭，文件级可进入 AU-04；剩余完整 empty/work-only/failure UI、显式 archived source、AU-07 完整 replay/trace 和搜索/压缩细化登记为 P1/P2。AU-04 高风险确认主链已由 `au04-confirm-before-execute` 登记为质量场景，重复确认不重复执行已由 `au04-confirm-idempotency-ui` 证明，当前-turn 旧确认 stale 拒绝已由 `au04-stale-confirmation-ui` 证明，expired confirmation 拒绝已由 `au04-confirmation-ttl-ui` 证明，disabled confirmation 不可提交已由 `au04-disabled-confirmation-action-ui` 证明，历史只读 confirmation 不可执行已由 `au04-history-confirmation-readonly` 证明，cross-work confirmation 隔离已由 `au04-cross-work-confirmation-guard` 证明，latest-context rebase 已由 `au04-latest-context-rebase-confirmation` 证明，ConfirmationBinding re-gate refs 已有强契约；持久 snapshot 和 replay 仍未闭环。AU-07 已补 why 入口、普通旧 turn scoped query、partial replay UI、gate reason why、scope negative matrix、StateTrace/adoption/projection replay producer、Behavior terminal replay producer、只读工具链 summary-level ToolTrace refs 和 ReplayReport 六问 cross evidence，二轮口径为 13/16 已验收、0/16 已测试、3/16 部分实现，P0 已关闭，文件级可进入 AU-08；剩余 P1 为 developer 双视图、完整 ToolTrace registry snapshot / redacted I/O、多类型 replay UI 矩阵和 reason catalog 深化，旧 workspace_id 迁移细化登记为 P2。AU-12 works 立项概览显示侧、accepted/tentative 状态、空字段、读取失败诚实降级、tab 导航、跨作品隔离、no-write、UUID 脱敏和 correction 修订意图回到对话/采纳边界均已有真实 Tauri / quality acceptance，文件级 P0/P1 已关闭；A2 同轮 provider prompt proof、accepted `world_setting` 物化 works 字段和立项要素深扩展为 P2。E2E-01 已按真实证据更新为 12/13 已验收、1/13 已测试、0/13 部分实现；E10 trace persister → SQLite → `TraceRepository.list_by_turn` 已补局部 integration proof，`e2e-01-full-chain` 聚合 runner、`e2e-01-downgrade-real-page`、`e2e-01-readonly-tool-trace`、`e2e-01-replay-report` 和 `e2e-01-channel-action-security` 已挂入 quality acceptance 并通过，P0/P1/P2 已关闭；E10 普通旧 turn scoped query、partial replay 和 scoped negative matrix 已由 AU-07 回填，developer / 多类型 trace query UI 继续归 AU-07 cross-reference。AU-10 baseline/task_state/provider failure/reconnect/cancel/timeout 已有 checkpoint；完整异步 LongRunner 因缺真实生产消费者按 `docs/design/04a-planning-and-long-run.md` 延后。当前继续铺产品功能广度，`AU09-archive-memory-roundtrip` CP2 已由 `au09-adopt-setting-recall` 证明真实页面伏笔/规则生成、adoption、tab 重开可见、recall/why；`AU09-memory-management-workbench-entry` 已由 `au09-memory-management-entry` 证明正式入口与基础生命周期治理；`AU09-memory-trace-roundtrip` 已补 lifecycle/reference author-safe 追溯；`AU09-validity-window-recall` 已补章节有效期窗口；`AU09-cross-work-memory-isolation` 已补跨作品隔离；`AU09-AU03-session-memory-layering` 已补同一作品内 active/historical session 与 governed memory 分层；`AU11-quality-diagnosis-message-envelope` 已补 SC-AU11-01 最小真实工作台 trace proof。**
...
> 2026-06-22 补充：AU-07 二轮缺口收敛已按依赖关系关闭旧 turn persisted trace query P1、partial replay UI P1、action_scope gate reason why P1 和 scope negative matrix P1；7 个 AU-07 本体真实 Tauri / quality 入口已有当前证据：`au07-trace-why-entry`、`au07-persisted-trace-query`、`au07-partial-replay-ui`、`au07-gate-reason-why`、`au07-trace-query-scope-negative-matrix`、`au07-state-trace-adoption-replay`、`au07-behavior-trace-terminal-replay`。其中 `au07-trace-query-scope-negative-matrix` 证明原 work/session/turn replay 200，跨 work、跨 session、same-work other session 和 missing turn 均 404，且不泄露 trace。`e2e-01-readonly-tool-trace` 与 `e2e-01-replay-report` 的默认 Tauri 和 real LM Studio quality 入口均通过，证明只读 `character_roster` 工具链的持久 `tool_trace_refs` 可查询，且 ReplayReport 可从持久 trace no-provider 生成完整六问。AU-07 二轮口径调整为 13/16 已验收、0/16 已测试、3/16 部分实现；developer 双视图、完整 ToolTrace registry snapshot / redacted I/O、多类型 replay UI 矩阵和 reason catalog 深化继续登记为 P1 owner，旧 workspace_id 迁移细化登记为 P2，当前可进入 AU-08。
>
...
> 2026-06-22 补充：AU-08 二轮缺口收敛发现并关闭 `p1-chapter-draft-generation` cross-reference driver 漂移。旧 driver 等待“待确认的创作材料”文案，当前真实 UI 为“待保存章节草稿/章节正文草稿”；修复只改外部 driver，并在点击真实“阅读”入口后等待新的 `channel.get_toc.done`。随后 `p1-chapter-adoption-reading`、`p1-chapter-edit-then-accept`、`p1-word-count-audit`、`p1-chapter-expansion-multichapter`、`p1-export-minimum`、`p1-chapter-draft-generation`、`au02-unadopted-candidate-no-reading-fact`、`su02-artifact-projection-trace-isolation` 均已串行复跑通过。本轮又新增 `au08-reading-readonly-no-write`，证明真实阅读模式查看、导出和返回期间不发送 `author_action` / `user_message`，不触发 adoption / tool execution / production write claim，并能返回可输入工作台；新增 `au08-reading-return-context`，证明从阅读返回工作台后 follow-up turn 继续使用同一 `work_id` / `session_id`，且返回动作本身 no `author_action` / no `user_message`。AU-08 当前口径为 12/16 已验收、2/16 已实现未验收、2/16 部分实现；当前无本文件内未关闭 P0/P1 blocker，可进入 AU-09；projection refresh/status machine 和失败降级仍登记为 P1 后续。
>
> 2026-06-21 补充：AU-09 已完成文件级收口，当时口径为 10/14 已验收、1/14 已测试、3/14 部分实现，P0 已关闭，当时可进入 AU-10。新增 `au09-memory-create-recall`、`au09-memory-management-entry`、`au09-memory-trace-roundtrip`、`au09-adopt-setting-recall`、`au09-character-dossier-roundtrip`、`au09-validity-window-recall`、`au09-cross-work-memory-isolation`、`au09-au03-session-memory-layering` quality manifest，并明确历史 `au09-archive-real-data` / `au09-memory-recall-context` 只作背景证据。2026-06-21 的剩余 P1/P2 为完整 archive stats、pending inbox、筛选分页真实页面矩阵、developer replay、历史旧 turn 查询、Channel 管理入口和完整独立 MemoryTrace/StateTrace 表。
> 2026-06-22 补充：AU-09 二轮缺口收敛新增并复跑 `au09-memory-management-filter-matrix`，证明真实工作台从“记忆”入口创建 3 条当前作品记忆，并通过可见 keyword/type/scope/status/locked 过滤和组合矩阵隔离匹配行，且请求 URL 携带可见 filter 参数。本轮随后新增并复跑 `au09-archive-stats-current`，证明真实工作台打开作品档案概览/伏笔详情/经验规则 tab 时，`get_work_stats/get_characters/get_foreshadowing/get_rules` 与 UI 统计同属当前 `work_id`，summary 记录 `archive_volumes=1`、`archive_chapters=1`、`archive_memory_items=3`、`archive_drafts_total=2`、`archive_drafts_accepted=1`、`archive_foreign_excluded=true`。10 个真实 Tauri / quality 入口已串行复跑通过：`au09-memory-create-recall`、`au09-archive-stats-current`、`au09-memory-management-entry`、`au09-memory-management-filter-matrix`、`au09-memory-trace-roundtrip`、`au09-adopt-setting-recall`、`au09-character-dossier-roundtrip`、`au09-validity-window-recall`、`au09-cross-work-memory-isolation`、`au09-au03-session-memory-layering`。AU-09 当前为 14 个场景中 12 个已验收、2 个部分实现；普通旧 turn scoped query 与 partial replay UI 已由 AU-07 回填；pending inbox、developer replay、多类型 replay UI 和完整独立 MemoryTrace/StateTrace 表继续登记为 P1，排序/分页/limit UI 深化登记为 P2，当前可进入 AU-10。
>
...
> 2026-06-22 补充：AU-12 二轮缺口收敛首次复核未发现本文件内应关闭的新 P1/P0；2 个真实 Tauri / quality 入口已串行复跑通过：`au12-work-profile-overview`、`au12-work-profile-status-isolation`。AU-12 当时保持 11 个场景中 8 个已验收、1 个已测试、1 个部分实现、1 个未实现；读取失败诚实降级和 correction 修订意图当时继续登记为 P1，A2 同轮 provider prompt proof 为 P2 加强项，可进入 E2E-01。
> 2026-06-22 补充：AU-12 依赖排序下继续关闭 `SC-AU12-C2` correction 修订意图 P1。新增 `au12-correction-intent-roundtrip` 真实 Tauri / quality acceptance，证明作者从作品档案「概览」点击「提出立项修订」后，真实工作台发送 `generate_micro_plan=true`，Planner/MicroPlan/Orchestrator 允许 `world_building`，生成 pending `world_setting` 和保存/修改/放弃动作，作者选择前无 `author_action`、无 adoption evaluation、无 production write。AU-12 当时为 9/11 已验收、1/11 已测试、1/11 部分实现；剩余 P1 只保留读取失败诚实降级，accepted `world_setting` 物化 works 字段为 P2。
>
> 2026-06-22 补充：AU-12 继续关闭 `SC-AU12-B3` 读取失败诚实降级 P1。新增 `au12-profile-read-failure-degrade` 真实 Tauri / quality acceptance，证明外部停服后真实作品档案显示「作品档案读取失败」与「重试读取」，不显示 `状态未明` 或空字段行；服务恢复后点击重试读回真实 works 立项字段，全程 no `user_message` / no `author_action` / no adoption / no tool / no write。AU-12 当前为 10/11 已验收、1/11 已测试；P1 已关闭，剩余为 A2 同轮 provider prompt proof、accepted `world_setting` 物化 works 字段和立项要素深扩展 P2。
>
...
> 2026-06-22 security 补充：新增 `e2e-01-channel-action-security --surface tauri`，以真实 Tauri 页面作为 work/session anchor，通过外部协议 socket 证明客户端伪造 `source_turn_result` 不能授权 invented action，旧 `source_turn_ref` 在 current turn 推进后被 stale 拒绝；该入口通过后 `e2e-01-full-chain --provider lmstudio` 聚合 runner 更新为 12/13 已验收、1/13 已测试，E8/E13 P2 Channel security regression 已关闭，E10 继续归 AU-07 cross-reference。
