...
> 2026-06-20 补充：AU-04 新增 `au04-confirmation-tool-failure-recovery` 真实 Tauri / quality acceptance 证据，证明确认后 `prose_writing` 工具失败会显示失败且无 pending draft / production write。AU-04 当前口径为 9/18 已验收、5/18 已测试、4/18 部分实现；剩余为持久 ConfirmationBinding snapshot、replay、LLM timeout/retry action 和 LongRunner 全矩阵。
