...
- `ADR-0001` 已经将 Orchestrator 裁决所依赖的 frame 语义升级为 Accepted 决策。
- `ADR-0003` 已经提出 Orchestrator 审查 Planner 建议时依赖的权限边界。
- `ADR-0004` 已经提出 OrchestratorDecision 的最小 contract。
- `ADR-0005` 已经提出 decision 形成前的 gate 顺序。
- `tasks/slices/v3/DAG.md` 已经把 Orchestrator 相关 proof 放入首批 slice。
