...
23. 结构面板字段优先级由 `../domain/34-novel-element-field-priority.md` 冻结，渐进披露由 `../ui/43-structure-panel.md` §4 约束；首批结构面板对象、三层优先级、L1-L4 渐进披露与 UI 不得自造 schema / 子类型 / 编辑表单的边界已收口。
