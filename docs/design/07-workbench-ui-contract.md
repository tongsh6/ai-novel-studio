...
> 状态：草案（2026-05-02）
>
> 角色：定义当前 Workbench UI 如何消费 TurnResult、BehaviorState、available actions、ui_cards、trace summary、projection hints，以及 UI 事件如何回到 AuthorInput / ActionInput。本文是 ADR 前的 UI contract 草案，不是视觉稿、组件实现或前端技术方案。
>
> 相关文档：
>
> - `00-vision-and-engineering-roadmap.md` — 当前愿景与工程推进方式
> - `00a-reading-map.md` — 当前阅读路径
> - `00b-end-to-end-dialogue-flow.md` — 一次 turn 的动态主链
> - `00c-state-and-contract-atlas.md` — 状态、contract、ADR 候选和 slice 入口索引
> - `00d-runtime-architecture.md` — 当前运行时架构视图
...
当前 UI 可以支持 streaming，但 streaming 不改变 canonical contract。
...
| ADR | 冻结内容 |
|---|---|
| TurnResultViewModel | UI 主消费 envelope |
| UI Card Types | card_type 集合与字段 |
| AvailableAction | action envelope、source_turn、behavior_ref、target_ref |
| Trace Summary Visibility | trace summary 层级与 redaction |
| Candidate Selection UI | candidate / selection / adoption 分界 |
| Streaming UI Contract | draft 与 final TurnResult 分界 |
| Projection Hint UI | projection refresh 与写入边界 |

ADR 前置材料已经具备：

1. `00c-state-and-contract-atlas.md`

原因是 UI contract 已经把当前主链输出对象串到作者前台，`00c-state-and-contract-atlas.md` 已经汇总 contract、状态、ADR 与 slice 入口。
...
- `adr/README.md` 已经建立当前 ADR 编号、状态、模板和首批 ADR 顺序。
- `ADR-0001` 已经将 UI 可引用的 frame 语义来源升级为 Accepted 决策。
- `ADR-0003` 已经提出 UI action 上游 Planner 只有建议权、没有执行批准权。
- `ADR-0004` 已经提出 UI action 上游的裁决表达。
- `ADR-0005` 已经提出 UI action 暴露前的 gate 顺序。
- `tasks/slices/DAG.md` 已经安排 UI action roundtrip 在主链 proof 之后。
