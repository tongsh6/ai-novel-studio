...
- **三防护（逐条硬性）**：①显式标注——注入文本以"【暂定】"前缀+依据摘要，模型与
...
```
盘点/分析产出 → 一致性校验 → PROPOSED（面板「暂定设定」区可见）
作者无动作 + 系统需要 → ACTIVE（开始注入，【暂定】标注）
...
| 面板 | 「暂定设定」区（AU-12"待确认"范式复用）+负债 finding 进审读报告卡（既有载体）；
  AU-12/AU-13 边界重画见 §6 |
...
- **OQ2 假定 PROPOSED→ACTIVE 放行 = 分级**（用户拍板）：required 事实的假定自动 ACTIVE+即时通知；recommended 事实的假定等作者放行。
