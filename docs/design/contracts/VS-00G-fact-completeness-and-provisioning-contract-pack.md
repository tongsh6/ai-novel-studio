...
真空变体（D1，2026-08-25）：角色档案**显式为空**（roster: []）时上述守则是死锁令
（无名可用又禁止取名），换 `protagonist_missing_vacuum`：

> 本作品尚无任何角色档案：请为出场人物取用稳定的具体名字并全文保持一致，不得用「主角」
> 「反派」这类叙事标签指称人物；人物档案将在写作后提请作者确认。

MBC 判例：为旧模型失败模式（gpt-oss 凭空造角）建的防线把守指令的 qwen3.8 逼进镜像失败
（正文写「主角」，M3 标本 166 章 0 处 vs M5 41 处）——**换模型族后防线须复验**。
快照缺 roster 数据源时维持原守则（无法确认≠确认为空）。
