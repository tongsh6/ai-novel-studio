...
Adoption Decision Card checkpoint（2026-05-15 14:14）：已补第一步状态化展示。`WorkspaceChat` 会从完整 transcript 与 resume snapshot 的 `adoption_state.resolved[]` / `resolved_adoptions` 建立 resolved artifact 视图；旧 `adoption_card` 若命中同一 artifact，不再渲染采纳/废弃/修改按钮，而是渲染为作者可见的决策记录，显示 `已采纳` / `已废弃` / `已修改后采纳` 及对应说明。右侧档案 pending 统计会按 `artifact_id` 去重，并排除已 resolved artifact，避免重开后继续显示 `待采纳 2`；已处理卡也不再复用“待确认的新设定”等待处理态标题，fallback 为中性的“创作设定”。实现仍复用 canonical `adoption_card`，不新增 card_type，也不改后端 adoption boundary。验证：`pnpm --dir frontend typecheck`、`pnpm --dir frontend lint`、`pnpm --dir frontend test`、`bash scripts/check_design_trace.sh`、`bash scripts/frontend_audit.sh`、`bash scripts/ai_static_scan.sh --top 10` 均完成；frontend audit 仅保留既有 `works.ts` localStorage 与本机 DMG bundle warning。剩余产品化动作：为已采纳内容补“打开阅读模式/查看已采纳内容/继续扩写”等后续动作，并把阅读投影提示统一成同一 resolved 视图。
