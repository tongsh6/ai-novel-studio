...
AU-09 Memory Recall Context checkpoint（2026-05-19）：已补齐 confirmed/stabilized + recallable 记忆进入真实对话 context 的最小闭环。新增 `NovelPersistence.MemoryRecallRepo`，按当前 `work_id`、记忆状态、`recallable=true` 和本轮作者输入的基础 lexical overlap 做召回排序；`WorkspaceContext.context_fetcher_with_query/0` 返回 `memory_summary` 并写入 `memory_reference_logs`、递增 `reference_count` / `last_referenced_at`。`DialogueGateway` 调用 `ContextAssembler.assemble_for_input/3`，把作者文本传给真实 fetcher，同时兼容旧一参 fetcher；`ContextAssembler` 日志增加 `has_memory`，`trace_summary.context_refs` 保留 `source_type=:memory`。新增原生 Tauri 验证 `au09-memory-recall-context`：seed 当前作品 confirmed memory 后，从真实工作台输入“林烬为什么要去灵源矿区？”，验证同一 turn 完成 `channel.user_message.start → context.assemble.done(has_memory=true) → planner.form_frame.done → channel.user_message.done` 且未触发 MicroPlan。证据：`artifacts/slice-verify/au09-memory-recall-context-tauri/summary.json`；task-done manifest 已生成并通过检查。验证：`mix compile --warnings-as-errors`、`mix test`、xref cycle check、arch check、`pnpm --dir frontend typecheck`、`pnpm --dir frontend lint`、`pnpm --dir frontend test`、`bash scripts/check_design_trace.sh`、`bash scripts/frontend_audit.sh`、`pnpm tauri build`、`bash scripts/tauri_slice_verify.sh au09-memory-recall-context`、`bash scripts/task_done.sh --slice au09-memory-recall-context --skip-static-scan`、`bash scripts/ai_static_scan.sh --top 10` 均通过；AI 静态扫描 14 PASS / 0 finding。剩余：AU-09 仍未完成 memory REST/Channel 管理入口、面板内采纳进入 governed memory、作者可见引用溯源 UI、locked/状态机 guard、有效期窗口、跨作品/历史会话分层验收；本 checkpoint 不能把 AU-09 整体标 done。
