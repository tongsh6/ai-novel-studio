...
| P0 | 候选方向操作闭环 | 模糊输入 → 候选方向 → 点选某方向继续探索 → 明确采纳时进入 adoption boundary | AU-02/AU-05/AU-10 | 已闭环 | `au02-candidate-continuation` 与 `au02-candidate-adoption-bridge` 已提供真实 Tauri 证据；后续转 AU-05 safety/freshness |
...
| AU-02 候选方向状态已重算 | backend fallback/real_llm 证据已进入验收文档；候选卡点选和采纳桥接已闭环 | 下一步转 AU05-GAP-06/07：freshness/conflict/cross-work 和高风险 confirmation |
