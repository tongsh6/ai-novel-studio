...
| P0 | 候选方向操作闭环 | 模糊输入 → 候选方向 → 点选某方向继续探索 → 明确采纳时进入 adoption boundary | AU-02/AU-05/AU-10 | 已闭环 | `au02-candidate-continuation` 与 `au02-candidate-adoption-bridge` 已提供真实 Tauri 证据；高风险候选 confirmation checkpoint 见 `au05-adoption-safety-freshness`，后续转 stale/conflict/cross-work |
...
| AU-02 候选方向状态已重算 | backend fallback/real_llm 证据已进入验收文档；候选卡点选、采纳桥接和高风险候选 confirmation checkpoint 已闭环 | 下一步转 AU05-GAP-06：freshness/conflict/cross-work |
