...
| 当前状态 | 最小真实前后端闭环已完成 |
| 当前缺口 | 高风险 confirmation 和 stale restored candidate rejection 已有真实 Tauri checkpoint；conflict、cross-work 采纳安全仍待 AU05 safety/freshness 加固 |
| 优先级 | P0 |
...
| 当前缺口 | context version / state snapshot / revision freshness 仍未形成完整持久化 contract；conflict/cross-work 仍需真实 UI 验收 |
| 优先级 | P0 |
...
**结论：18 个场景；已有草稿采纳/放弃/修改、候选继续探索、候选授权采纳、高风险候选 confirmation checkpoint 和 stale restored candidate rejection checkpoint 等真实 Tauri 前后端闭环；关键剩余缺口集中在 StateTrace、持久化作品事实、阅读投影刷新状态机，以及 conflict、cross-work 采纳安全。**
...
| AU05-GAP-06 — freshness / conflict / cross-work 检查不足 | restored stale candidate 已有真实 Tauri 拒绝证据；cross-work rejection 已有 application 回归但缺真实 UI 验收；缺 context version、revision、conflict recovery | 补实现/补测试/补验收 | P0 |
...
4. stale/conflict/cross-work：已证明恢复出的 stale candidate 不能写入当前作品；还需补冲突草稿、跨作品草稿不能写入当前作品。
