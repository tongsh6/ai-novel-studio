...
### D2-024 Maintenance Artifact + Adoption Card Payload 最小 schema 由 ADR-0007 冻结

Maintenance artifact 与 adoption card payload 最小 schema 已通过 ADR-0007（`adr/0007-maintenance-artifact-schema.md`）冻结。maintenance hook 产物以 `maintenance_artifact` 挂载到 `TurnResult.adoption_state.pending[]` 中 `artifact_adoption_entry.domain_ext.maintenance` 子对象，必填字段为 `hook_name` / `revision_base` / `proposed_change` / `requires_adoption` / `adoption_status` / `risk_class`，可选 `auto_adoption_hint` / `target_scope_ref` / `validator_findings`；`adoption_status` 复用 ADR-0001 line 318 `artifact_adoption_entry.adoption_status` 命名（与容器 `TurnResult.adoption_state` 显式区分），取值严格使用 §3.2 7 态，不引入 `AUTO_ADOPTED` 等非 canonical 值。UI 投影使用 ADR-0006 已冻结的 `card_type=adoption_card`，payload 必填 7 字段（`artifact_id` / `artifact_type` / `hook_name` / `revision_base` / `risk_class` / `diff_summary` / `target_object_type`），actions 至少包含 `accept` / `edit_then_accept` / `discard`。`auto_adoption_hint=true` 不授权系统跳过 adoption boundary；具体自动 adoption 决策策略由后续 W9 工单与 `32-human-approval-policy.md` `default_behavior` 映射联合冻结。
