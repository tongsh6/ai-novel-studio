...
14. Card / action 最小 schema 由 ADR-0006 冻结；card 公共字段 9 必填 + 1 可选 `payload`，字段集与 `11-ux-contract.md` §6.1 对齐；Domain 扩展使用 `domain_ext.` 前缀；`action_type` / `affordance_kind` / `next_action` 边界清晰，W4 不发明新 `next_action` 值。
15. Maintenance artifact + adoption card payload 最小 schema 由 ADR-0007（`adr/0007-maintenance-artifact-schema.md`）冻结；`maintenance_artifact` 挂载到 `TurnResult.adoption_state.pending[]` 的 `artifact_adoption_entry.domain_ext.maintenance`，`adoption_status` 复用 ADR-0001 line 318 `artifact_adoption_entry.adoption_status` 命名（与容器 `TurnResult.adoption_state` 显式区分）并严格使用 §3.2 7 态，不新增枚举值；UI 投影使用 ADR-0006 已冻结的 `card_type=adoption_card`，payload 必填 7 字段（见 ADR-0007 §3）；auto-adoption 由 `auto_adoption_hint` (boolean) + `risk_class` 提示，但策略由后续 W9 工单冻结，不绕过 adoption boundary。
...
20. Quality finding 最小 schema 与 UI 投影由 ADR-0012（`adr/0012-quality-finding-ui-projection.md`）冻结；`quality_finding.action` 是 Domain policy decision，不是 runtime `NextAction`；finding 不直接修改 artifact 或 authoritative object，只作为 policy、adoption、checkpoint、UI card 和 Experience Engine 输入。
21. Approval policy / approval record 最小 schema 与 UI 投影由 ADR-0013（`adr/0013-approval-policy-record-ui-projection.md`）冻结；`risk_class` 最小枚举为 `LOW` / `MEDIUM` / `HIGH` / `CRITICAL`；`default_behavior` 必须映射到 ADR-0002 `NextAction`，但不等同于 `NextAction`；bypass 不得覆盖 authority / revision / security / hard consistency conflict。
