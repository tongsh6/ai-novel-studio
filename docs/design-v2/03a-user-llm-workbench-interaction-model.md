...
> **现状**：工作台挡在用户和 LLM 之间当柜员。
> **目标**：用户通过 LLM 进行创作，工作台是 LLM 的装备库、技能库、武器库。

本文诊断：**这是设计问题，而非实现问题**。根因在于 `Router` + `TurnService` 在交互链路上的位置错误——工作台抢了 LLM 的位置。
...
工作台充当了**用户和 LLM 之间的柜员**——先拦截用户输入，提取结构化参数，验完整了才放行给 LLM。LLM 不参与对话引导，只是最后被叫来执行内容生成的工具。工作台本应是 LLM 的武器库，却反过来把 LLM 当成了自己的武器库。
...
ADR-0010 的 slot schema **本身不是问题**——定义执行前需要什么参数是正确的。它属于**装备库**里的参数定义，LLM 用它来理解"我需要收集什么信息"。问题是当前 slot 校验被放在了**用户面前**（Router 拦截追问），而不是**LLM 身后**（LLM 对话收集完后，工作台在后台校验）。
...
核心隐喻：**用户通过 LLM 进行创作，工作台是 LLM 的装备库、技能库、武器库。**

```
用户 ──对话──→ LLM（创作伙伴：理解意图、展开讨论、探索方向、生成内容）
                 │
                 │  LLM 需要什么，就从工作台取什么
                 │
                 └── 装备 ──→ 工作台 ──→ 装备
                     ...
                     【装备库】上下文、记忆、当前 slot 状态、作品信息
                     【技能库】intent 执行、内容生成、slot 校验
                     【武器库】数据持久化、adoption、projection、audit
```

LLM 是**使用者**，工作台是**被使用的工具箱**。用户看不到工作台——用户只看到 LLM，只和 LLM 对话。LLM 需要查状态、写数据、校验参数时，伸手到工具箱里拿。

### 3.1 角色重新定义

| 角色 | 当前定位 | 目标定位 |
|------|---------|---------|
| 用户 | 回答 slot 问题 | 与 LLM 自然对话，被引导着把想法说清楚 |
| LLM | 被调用做内容生成（最后一步） | **创作伙伴**：对话引导 + 意图理解 + 参数探索 + 内容生成 |
| 工作台 | 拦截 + 校验 + 追问用户 | **LLM 的工具箱**：装备（上下文/状态）、技能（intent 执行）、武器（持久化/门禁） |
...
│  【武器库】                               │
│  ─ 数据持久化（写 DB）                     │
│  ─ adoption（artifact 生命周期）           │
│  ─ projection（阅读投影刷新）               │
│  ─ audit / replay（留痕）                  │
│  ─ confirmation gate（高风险门禁）          │
│                                          │
│  【技能库】                               │
│  ─ intent 执行（CREATE_WORK_SEED 等）      │
│  ─ slot 校验（blocking_slots 硬门禁）      │
│  ─ 内容生成（draft/chapter/scene）         │
│  ─ 候选方向生成（探索式引导）               │
│                                          │
│  【装备库】                               │
│  ─ 当前作品上下文（work/volume/chapter）     │
│  ─ 对话历史 + accumulated_slots            │
│  ─ memory recall（世界观/角色/伏笔）         │
│  ─ slot schema（参数定义）                  │
│  ─ authority / budget（权限预算）           │
│                                          │
└──────────────────────────────────────────┘
```

三层的关系：LLM 从**装备库**读取上下文判断当前状态 → 调用**技能库**执行操作 → **武器库**是技能中需要持久化或受管控的部分。
