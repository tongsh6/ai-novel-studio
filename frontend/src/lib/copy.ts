...
export const WORKBENCH = {
  ...
  agentRunInlineStatus: (status: string) => `当前：${status}`,
  agentRunFallbackStatus: (status: string) => `当前创作请求${status}`,
  agentRunFlowAriaLabel: "当前创作请求的工作过程",
  agentRunControlsLabel: "创作请求控制",
  agentRunDetailsSummary: "工作详情",
  agentRunDetailsHint: "理解、计划、执行与候选边界",
  agentRunDetailsLoading: "正在加载工作详情",
  ...
  agenticLoopTerminalLabels: {
    turn_result_ready: "回应已就绪",
    run_completed: "已完成",
    run_cancelled: "已取消",
    run_failed: "失败",
    awaiting_author: "等待作者",
  } as Record<string, string>,
  ...
  agenticLoopPlanStepKindLabels: {
    explore: "探索",
    act: "行动",
  } as Record<string, string>,
  agenticLoopPlanStepStatusLabels: {
    pending: "等待",
    active: "进行中",
    done: "完成",
    skipped: "跳过",
  } as Record<string, string>,
  agenticLoopPhaseLabels: {
    plan: "规划",
    explore: "探索",
    evaluate: "评估",
    replan: "重规划",
  ...
  agentRunProviderRunsLabel: "模型调用明细",
  agentRunProviderReplayEventsLabel: "事件序列",
  agentRunProviderReplayOutputLabel: "输出摘要",
  agentRunProviderReplayBoundary: "回放边界：只读取已保存的 ProviderRun 事实，不会重新调用模型。",
  agentRunProviderReplayNoOutput: "暂无输出摘要",
  agentRunProviderReplayEventFallback: (index: number) => `事件 ${index}`,
  agentRunProviderReplaySequence: (sequence: number) => `序号：${sequence}`,
  agentRunProviderReplayEventType: (eventType: string) => `事件类型：${eventType}`,
  agentRunProviderReplayEmittedAt: (time: string) => `发生于：${time}`,
  agentRunProviderReplayFinalizedAt: (time: string) => `完成于：${time}`,
  agentRunProviderReplayRefs: (refs: string) => `引用：${refs}`,
  agentRunBriefPath: (path: string) => `本轮路径：${path}`,
  agentRunBriefProviderCalls: (count: number) => `模型调用 ${count} 次`,
  ...
  agentRunBriefPathLabels: {
    intent: "理解作者意图",
    profileSelected: "选择工作流",
    profileConversation: "进入对话回应工作流",
    profileCharacterDesign: "进入角色设计工作流",
    profileProseDrafting: "进入正文写作工作流",
    profilePlotOutline: "进入章节大纲工作流",
    profileCharacterEvolution: "进入角色演化工作流",
    profileWorldBuilding: "进入世界设定工作流",
    profileRevision: "进入修订候选工作流",
    profileProviderProgress: "进入模型执行工作流",
    profileReadonlyBatch: "进入只读上下文工作流",
    context: "读取上下文",
    providerConversation: "调用模型",
    providerPlanning: "调用模型规划",
    providerAgentPlanner: "调用步骤规划模型",
    providerWriter: "调用写作模型",
    providerEvaluator: "调用复核模型",
    providerRevision: "调用修订模型",
    providerTool: "调用工具模型",
    providerNarration: "调用回应模型",
    providerOther: "调用模型",
    modelJudgment: "模型判断",
    planCreated: "制定计划",
    systemGate: "系统裁决",
    toolExecution: "执行创作能力",
    plotOutlineTool: "规划章节大纲",
    characterEvolutionTool: "更新角色演化记忆",
    worldBuildingTool: "构建世界设定",
    qualityReview: "质量复核",
    tentativeArtifact: "生成待采纳候选",
    plotOutlineArtifact: "生成大纲候选",
    characterEvolutionArtifact: "生成角色演化候选",
    worldBuildingArtifact: "生成设定候选",
    response: "完成回应",
    cancelled: "已取消",
    failed: "执行失败",
  } as Record<string, string>,
  ...
  agentRunCancellingSummary: "正在取消这次创作请求。",
  ...
  agentRunDetailContextRefs: (count: number) => `引用了 ${count} 条上下文依据`,
  agentRunDetailFrameType: (frameType: string) => `认知帧：${frameType}`,
  agentRunDetailNeedsTool: (needsTool: boolean) =>
    needsTool ? "判断需要进入执行门禁" : "判断无需工具执行",
  agentRunDetailCandidateCount: (count: number) => `候选方向：${count} 个`,
  agentRunDetailPlanRef: (ref: string) => `计划编号：${ref}`,
  agentRunDetailActionCount: (count: number) => `计划动作：${count} 个`,
  agentRunDetailDecisionRef: (ref: string) => `裁决编号：${ref}`,
  agentRunDetailDecisionType: (type: string) => `裁决结果：${type}`,
  agentRunDetailBlockingGate: (gate: string) => `阻塞门禁：${gate}`,
  agentRunDetailProfileRef: (profile: string) => `任务类型：${profile}`,
  agentRunDetailProfileSource: (source: string) => `选择来源：${source}`,
  agentRunDetailProfileReasons: (reasons: string) => `选择原因：${reasons}`,
  agentRunDetailProfileTerms: (terms: string) => `命中词：${terms}`,
  agentRunDetailProviderEvent: (event: string) => `模型事件：${event}`,
  agentRunDetailProviderPurpose: (purpose: string) => `用途：${purpose}`,
  agentRunDetailProviderStatus: (status: string) => `状态：${status}`,
  agentRunDetailProviderOutputType: (type: string) => `输出类型：${type}`,
  agentRunDetailProviderRunRef: (ref: string) => `运行编号：${ref}`,
  agentRunDetailProviderCallRef: (ref: string) => `调用编号：${ref}`,
  agentRunDetailProviderPhase: (phase: string) => `进展阶段：${phase}`,
  agentRunDetailProviderContentLength: (length: number) => `结果长度：${length} 字`,
  agentRunDetailProviderChunkIndex: (index: number) => `片段序号：${index}`,
  agentRunDetailProviderChunkLength: (length: number) => `片段长度：${length} 字`,
  agentRunDetailProviderAccumulatedLength: (length: number) => `已接收：${length} 字`,
  agentRunDetailProviderUsage: (tokens: number) => `用量记录：${tokens} tokens`,
  agentRunDetailProviderInputUsage: (tokens: number) => `输入用量：${tokens} tokens`,
  agentRunDetailProviderOutputUsage: (tokens: number) => `输出用量：${tokens} tokens`,
  agentRunDetailProviderModel: (model: string) => `模型：${model}`,
  agentRunProviderEventLabels: {
    started: "开始调用",
    progress: "接收进展",
    chunk: "接收片段",
    final_output: "收到结果",
