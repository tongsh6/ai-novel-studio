...
from novel_workbench.executors import ExecutorRegistry, FunctionExecutor, make_executor_result
...
class ExecutorRegistryTest(unittest.TestCase):
    def test_registry_dispatches_registered_executor(self) -> None:
        ...
        def handler(*, context, parameters):  # type: ignore[no-untyped-def]
            return make_executor_result(handled=True, status="COMPLETED", action_result={"ok": True, "context": context, "parameters": parameters})

        registry.register(FunctionExecutor("TEST_INTENT", handler))
