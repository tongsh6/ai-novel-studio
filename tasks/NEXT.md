...
**收口清单（下一会话）**：①质量标准 4.3 核对 + 8 章未达标残留处置 → P1
里程碑关闭裁决；②clip_echo 修复便宜验证（单测截断形状断言 + 长摘要形态
端到端 live 单场景，探针短叙事覆盖不到）——**狗粮级清算按攒批模式搭下次
节拍跑，不为单个修复单独起跑**（用户 2026-07-21 确认；待清算批次=clip_echo
指标 wrong_route_reply 97→应降数量级 + 库迁移 --resume 跨重启实证 + B8/B7
实现后各自指标）；③郑果草稿废弃（新库已无此数据，条目可销）；④全部改动
未提交（judgment_protocol 目录注入+clip_echo、探针体温计、test.exs
NOVEL_TEST_DB_DIR、dogfood_run.sh、next_task_check、mint、文档）。
...
| 5 | M2 | A | P1 百章验收跑 + 质量标准 4.3 核对 + 导出 | **next**（2026-07-21 达标跑完成：101,421 字/75 章/导出 ✓；余质量 4.3 核对 + 8 章未达标处置 + clip_echo 修复验证，完成后 P1 关闭） | P1 里程碑关闭 |
