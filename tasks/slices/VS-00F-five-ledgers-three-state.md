...
| CP4 | 最小实现步：**CP4a** 规划消费账面（plot_outline 注入五账规划摘要+延续性要求，progress_state 同通道；prose 文案随迁 app 侧）；**CP4b** ledger_reconciliation_v1 profile 化+显式发起；**CP4c** Ledgers 面板+作者裁决入口[原 CP2c-2]+revise 生成联动+SC-AU13-B1/B2/C1（前端批） | 下次百章狗粮：漂移 30 章内被拦截 | **CP4a done**（2026-07-21：投影渲染单测 2 项+plot_outline flow 注入 ledger_reader；产品级效果随下次狗粮清算）；CP4b/CP4c 未开工 |
