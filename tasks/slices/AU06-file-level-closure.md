...
| SC-AU06-B2 点击取消/拒绝关闭 behavior | 已验收 | 完整 replay UI / developer view 仍缺 | 补验收 | P1 | `au10-workbench-recovery-cancel-waiting`、`au07-behavior-trace-terminal-replay` | 补旧 turn trace 查询 API/UI 与 developer 双视图 | 否 | AU-07 trace/replay | 是 |
...
| SC-AU06-D1 行为生命周期可回放且不调 LLM | 部分实现 | cancel terminal replay producer 已补；confirm terminal replay、旧 turn trace 查询 API/UI、developer 双视图仍缺 | 补集成/补验收 | P1 | `au07-behavior-trace-terminal-replay` | 补旧 turn trace 查询 API/UI 和完整 BehaviorTrace replay | 否 | AU-07 文件 owner | 是，已有 AU-07 cross evidence 支撑退出 |
