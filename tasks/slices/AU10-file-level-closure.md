...
| SC-AU10-E1 projection hint / 阅读模式 | 已验收 | 阅读模式查看/导出/返回 no-write 已由 AU-08 D1 关闭；refresh/rebuild/failed 专项仍归 AU-08 P1 | cross-reference | P1 | `p1-chapter-adoption-reading`、`p1-export-minimum`、`su02-artifact-projection-trace-isolation`、`au08-reading-readonly-no-write`、`au10-workbench-matrix-layout` | AU-08 projection refresh/status matrix | 否，AU-08 owner | AU-08 projection refresh/status matrix | 是 |
