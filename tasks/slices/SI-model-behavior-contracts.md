...
| T7 | MBC judgment-protocol 探针（ADR-0025 CP1 前置） | done（2026-07-16） | `scripts/model_contracts/judgment_protocol.exs`：验证判断①方案 B 回复内联协议（call1 叙事+直接回复同流、call2 轻量 `judgment_decision` tool call）与"是否开计划"判断质量（六用例阵列：闲聊/上下文事实/单动作创作/多步复杂/需检索/意图不明）。判断①提示词为 CP1 协议草案，CP1 落地时迁入生产模块。`NovelAgent.Provider.Stub` 新增判断契约样本（五选一确定性判形态 + 回复内联）。**测量方法定稿（双指标分账）**：多批次实测发现单一 pass 混淆了两类信号——协议合规（可工程保证）与判断准确率（模型固有方差，跨批 0.889~1.0 摇摆，低温 0.2/0.4 未收敛）。定稿口径：`protocol_compliance`（调用可解析+叙事存在，**硬闸门恒 1.0**）、`form_adherence`（内联两段形）、`judgment_accuracy`（阈值默认 0.85，ADR-0025 本以预算 backstop 兜底判断方差）。**终验（gpt-oss-120b，3 轮/用例）：protocol=1.0 / form=1.0 / judgment=0.944**，exit 0。测量迭代史：判断边界经"产物即单动作/多产物才开计划"判别规则加固（single_creative 0.33→1.0）；multi_step 接受 plan\|explore（先探索是合理首步）；残余摇摆为 multi_step 偶判 reply（~5%，CP1 运行时无害面：作者可改述+预算兜底）。stub 自检 12/12=1.0。判断调用温度定为协议属性（叙事 0.4/结构 0.2，`Execution.with_params/2` 新增）。证据 `artifacts/model-contracts/lmstudio/judgment-protocol.json`。另记两个观察：① config 默认 LM Studio 模型 id `qwen/qwen3.5-122b-a10b` 与服务端 served id `qwen3.5-122b-a10b` 不一致（JIT 加载 400），live 跑用 `NOVEL_LMSTUDIO_MODEL=openai/gpt-oss-120b`；② LM Studio adapter 400 错误 message 为空串（错误体未透出），排障靠 curl——可在 adapter 错误提取补 body 透出，未登记为独立任务。 |
