...
- 2026-06-15：**CP0 重设计**（commit c1c257d，初版瞄错靶子）。问题：planner 按规则把"未匹配列表的章"的 target_chapter 置 null，导致"续写第99章"静默回退到最新章；初版只挡 planner 回显坏章的罕见 case，漏主病灶。正确信号 = **作者点名（requested_chapter_raw 非空）+ planner 未匹配（target_chapter 空）**。
  ...
  - **tauri 外部场景已收口（2026-06-22 复核）**。harness 文件：`frontend/slice-verify/external-ui-driver.mjs`(driveCp0MissingChapterBlock)、`native-tauri-verifier.mjs`(vs00c-cp0-missing-chapter-block 事件)、`scripts/tauri_slice_verify.sh`(复用 seed_p1_chapter_draft_generation)。quality manifest 已补 `quality/acceptance/scenarios/vs00c-cp0-missing-chapter-block.yml`，当前入口 `bash scripts/quality_accept.sh vs00c-cp0-missing-chapter-block --surface tauri` 通过。
  - **旧诊断已 superseded**：2026-06-15 的 `authoring_mode=first_draft` / Tauri harness WIP 记录是当时运行态差异；当前真实 Tauri evidence 证明缺失目标章会在 provider/tool dispatch 前阻断，并返回诚实缺章回复，无 creative card / adoption card。
  - **CP0 当前交付**：后端修复、主链复现与真实 Tauri 页面验收均已闭合；后续只保留为 VS-00C CP0 regression。
...
- 2026-06-15（专调）：**根因基本定位**。in-process 复现完整应用主链：`DialogueGateway.handle_input/3`（直注 slice_verify complete_fn）→ **block 成功**（"没有找到《第99章》", tool_called=false）；进一步 `handle_input/2` 走 **`Gateway.complete` 配置 provider 路由**（= 真实 tauri 路径）、provider 配 **atom `:slice_verify`** → **同样 block 成功**。即 CP0 block 行为已穿过 handle_input→form_frame→Gateway.complete(slice_verify)→form_micro_plan→orchestrator→execute **整条主链 + 真实 Gateway 路由**验证通过，只差 Tauri React UI 渲染。
  ...
  - **2026-06-22 复核**：`bash scripts/quality_accept.sh vs00c-cp0-missing-chapter-block --surface tauri` 已通过，Tauri 页面证据不再是 tracked 缺口。CP0 产品行为本身已由 in-process 全链路和真实页面验收双重证明，无需再改后端。
