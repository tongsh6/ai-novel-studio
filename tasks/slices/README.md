...
| `AU05-file-level-closure.md` | **file-level deliverable / P1-P2 follow-up registered**：AU-05 已按文件级闭环口径复核为 18 个场景：15/18 已验收、3/18 部分实现。候选 adoption boundary、高风险 confirmation、stale rejection、cross-work/canon recovery、artifact accept/discard/edit_then_accept、未采纳/已放弃不进阅读、采纳后阅读 projection 和设定 recall 均有当前真实 Tauri / quality acceptance 证据；P0 已关闭，持久 adoption inbox、ProjectionHint stale/refresh、完整 StateTrace/replay 和 canon/revision store 自动计算已登记 owner；当前可进入 AU-06。 |
| `AU06-file-level-closure.md` | **file-level deliverable / P1 follow-up registered**：AU-06 已按文件级闭环口径复核为 17 个场景：13/17 已验收、4/17 部分实现。高风险 confirmation、普通聊天 no-waiting、active behavior UI、confirm binding/re-gate、cancel/reject、等待期间继续聊天 stale guard、旧/过期/禁用/历史/跨作品 action、TTL、幂等、single-active runtime safety 和前端只提交服务端 action 均有当前真实 Tauri / quality acceptance 证据；P0 已关闭，blocking clarification、confirm terminal history、BehaviorTrace/replay 与持久 BehaviorBinding ledger 已登记 owner；当前可进入 AU-07。 |
| `AU07-file-level-closure.md` | **file-level deliverable / P1 follow-up registered**：AU-07 已按文件级闭环口径复核为 16 个场景：6/16 已验收、4/16 已测试、6/16 部分实现。why 入口、StateTrace/adoption/projection replay producer、Behavior terminal close/resolution replay producer 均有当前真实 Tauri / quality acceptance 证据；P0 已关闭，旧 turn trace 查询 API/UI、developer 双视图、完整 ToolTrace registry snapshot、ReplayReport 六问和 work/session 查询隔离已登记 owner；当前可进入 AU-08。 |
| `AU08-file-level-closure.md` | **file-level deliverable / P1 follow-up registered**：AU-08 已按文件级闭环口径复核为 16 个场景：11/16 已验收、3/16 已实现未验收、2/16 部分实现。采纳正文、编辑后采纳、未采纳不入阅读、跨作品隔离、多章导航、空章诚实显示、短章 audit、导出和 STALE banner 均已挂入真实 Tauri / quality 入口；P0 已关闭，projection refresh 专用 action/no-write driver、REBUILDING/FAILED 状态来源、返回工作台上下文、只读 no-write 专项和离线降级登记为 P1；当前可进入 AU-09。 |
...
| `AU09-file-level-closure.md` | **file-level deliverable / P1-P2 follow-up registered**：AU-09 已按文件级闭环口径复核为 14 个场景：10/14 已验收、1/14 已测试、3/14 部分实现。8 个当前可复跑 Tauri 入口已挂入 quality acceptance，覆盖记忆创建确认召回、管理入口、生命周期终态排除、trace、伏笔/规则 adoption、角色主档案、有效期窗口、跨作品隔离和 AU-03 会话/记忆分层；P0 已关闭，完整 archive stats、pending inbox、筛选分页、developer replay、历史旧 turn 查询、Channel 管理入口和完整 MemoryTrace/StateTrace 登记为 P1/P2；当前可进入 AU-10。 |
| `AU10-file-level-closure.md` | **file-level deliverable / P1-P2 follow-up registered**：AU-10 已按文件级口径对账为 17 个场景：14/17 已验收、1/17 已测试、2/17 部分实现。6 个当前可复跑 AU-10 Tauri 入口已挂入 quality acceptance 并复跑通过；P0 已关闭，剩余 P1/P2 是完整异步 LongRunner、FAILED task_state 真实页面、全 card/action 深矩阵、projection/trace 深链路和验收卫生；当前可进入 AU-11。 |
| `AU11-file-level-closure.md` | **file-level deliverable / P1 follow-up registered**：AU-11 已按文件级口径对账为 4 个场景：2/4 已验收、2/4 部分实现。`au11-quality-diagnosis-message-envelope` 与 `au11-missing-workstate-policy` 已挂入真实 Tauri / quality acceptance，覆盖 VS-00D 质量诊断三层 envelope、缺上下文 WorkState missing、why 可见、不编造和 no tool/adoption/write；P0 已关闭，剩余 P1 是 `guidance_mode` schema 冻结、clarify/confirm 引导 proof、prose_writing envelope 投影；当前可进入 AU-12。 |
| `AU12-file-level-closure.md` | **file-level deliverable / P1 follow-up registered**：AU-12 已按文件级口径对账为 11 个场景：8/11 已验收、1/11 已测试、1/11 部分实现、1/11 未实现。`au12-work-profile-overview` 与 `au12-work-profile-status-isolation` 已挂入真实 Tauri / quality acceptance，覆盖作品档案概览、accepted/tentative 状态、空字段、tab 导航、跨作品隔离、no-write 和 UUID 脱敏；A2 prompt 同源为实现/测试约束，缺同轮 provider prompt 外部证据；P0 已关闭，剩余 P1 是读取失败诚实降级和 correction 修订意图；当前可进入 E2E-01。 |
| `E2E01-file-level-closure.md` | **file-level deliverable / P0-P1 closed, P2 security regression registered**：E2E-01 已按真实证据重算为 10/13 已验收、2/13 已测试、1/13 部分实现。真实 Tauri / real LM Studio 证据覆盖基础聊天、自然探索、上下文、执行降级、确认、只读工具调度、ReplayReport 六问、创作产出、错误恢复和两轮回路；E10 已补 DialogueGateway trace persister → SQLite → `TraceRepository.list_by_turn` integration proof；`e2e-01-full-chain`、`e2e-01-downgrade-real-page`、`e2e-01-readonly-tool-trace` 和 `e2e-01-replay-report` 已挂入 quality acceptance 并通过 `lmstudio` 验收。P0/P1 已关闭；P2 为 invented/forged source 恶意 payload 的 Channel security regression。 |
