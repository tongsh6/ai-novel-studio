...
| `AU09-memory-taxonomy-write-policy.md` | **todo / feedback 16,17**：记忆类型、角色记忆边界、写入/不写入/更新/废弃策略收敛。明确角色主体归 Character 主档案，角色记忆只承载演化、当前状态、关系变化等连续性事实。 |
| `AU09-memory-list-ux-redesign.md` | **todo / feedback 14**：记忆页面列表字体、行色、状态和扫描体验按全局 UI 调性重构；视觉语义必须服务真实 memory type/status，不改变召回或生命周期规则。 |
...
| `SU01-provider-vendor-matrix-openai-minimax-zhipu-kimi-gemini.md` | **todo / feedback 15**：扩展 OpenAI（`api_key` / `subscription`）、Minimax、智谱、Kimi、Gemini 供应商配置、模型列表、测试连接、secret redaction 和下一轮 runtime 切换矩阵；live vendor 验收需要真实账号策略，不能用假证据关闭。 |
