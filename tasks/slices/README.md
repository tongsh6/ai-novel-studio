...
| `UA01-bounded-run-single-candidate-termination.md` | **active / 僵尸 run 竞态收口**：bounded 创作 run 在 artifact_created 后必须诚实终止，不产出作者未请求的第二份候选（2026-07-04 狗粮长跑实锤：迟到帧误配/覆盖已累积正文/预算耗尽 awaiting_author/LM Studio 饿死后续 run）。T1 完成信号强制措辞、T2 runner turn 绑定已落地未复验；T3 机器强制方案（预算 vs contract 扩展 vs 提示词纪律）待用户决策；T5 长跑复验搭车下一次经批准的狗粮（0 retry 通过线）。 |
