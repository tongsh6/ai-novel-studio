...
| T3 | Router 升级：LLM-based intent 分类替代关键词识别 | done | 主路径 Provider.Gateway.complete；LLM 不可用时返回 `:unknown` 触发 clarification；新增 Router.Result 元数据字段 |
...
- 2026-04-30 — Router 统一使用 LLM 分类（Provider.Gateway）。LLM 不可用时返回 `:unknown`，触发 clarification 卡片告知用户，不做静默降级。
...
- Router LLM 分类在 LM Studio 未启动时返回 `:unknown` 并触发 clarification，告知用户 LLM 不可用。不做静默降级——产品不应在 LLM 不可用时冒充可用。
...
- LLM 分类失败时触发 clarification——用户有机会重新描述意图，这比任何本地 fallback 分类器更可靠。系统不应假装理解用户输入。
