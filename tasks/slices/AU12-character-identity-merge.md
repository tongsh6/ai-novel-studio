...
**状态**：**done（CP1/CP2 2026-08-10，CP3 2026-08-11）**；三拍板项已裁决（§8）；
一项狗粮观察缺口登记于 CP3 条目。
...
- **CP3 消费面证明 done（2026-08-11）**：
  ①**别称 sighting**：`ledger_maintenance_test` 既有「含别名入账」用例（摘要只提
  「浩子」→「林浩」入账）自 CP2 起从死分支测试变为真链路证据；**真实页面级未闭环**
  （确定性 fixture 不会自发用别称写正文，扭曲 fixture 违背「不朝 runner 硬编码」——
  缺的是真实模型自发用别称的正文，登记为 **M5 狗粮观察项**：长跑后查 arc 账
  source_refs 是否含别称章）。
  ②**阵容注入行升级**：`character_roster_line` 括号段 = 身份+别名
  （`- 沈洛（底层调查者，别名：洛公子）：…`，写作一致性——模型必须知道洛公子
  就是沈洛才不会把历史称呼写成新角色）；`character_roster_context_test` 补断言。
  ③**别名拦截卡真实页面**：au12 场景扩尾段——盘点把归并产生的别名「洛公子」
  当新人物重提（M4 行为的别名变体，替身按 prompt 已在档段落分支）→ 采纳被
  别名命中卡拦住并点名「『洛公子』是已确认角色『沈洛』的已登记别名」→ 作者
  拒绝 → 档案仍单行含别名、全场景零 accepted 采纳。**真实 Tauri 一轮 PASS**。
  归并阶段零 adoption 门收窄为阶段快照（拦截卡阶段合法产生 needs_confirmation
  评估）。
