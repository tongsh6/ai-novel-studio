...
- 状态：doing（AU-04 文件级 P0 closed；确认卡说明内容、高风险确认主链、重复确认幂等、当前-turn stale / expired / disabled / history-readonly / cross-work / latest-context rebase / confirmation tool failure recovery checkpoint closed；AU-06 single-active runtime safety closed；AU-06 clarification / replay / timeout matrix 继续）
...
- Invariant: UI 只能提交服务端 action；高风险执行确认前不得 tool dispatch 或 production write；确认卡必须解释确认对象、确认前 no-tool/no-write 和确认后重新 gate；确认/取消必须绑定 open behavior；确认后必须重新 gate，并留下 `rebased_state_snapshot_ref` / `gate_result_refs` proof；取消/拒绝必须关闭具体 behavior 且 no-write；同一 `idempotency_key` 不重复执行；当前 turn 已推进后旧 confirmation action 不能继续执行；超过 `available_action.expires_at` 的 confirmation 不能触发 re-gate、tool dispatch 或 pending draft；`enabled=false` 的 confirmation action 可以可见但不能提交到 `author_action` 边界；历史只读 transcript 内的 confirmation 不能渲染为可执行 action，也不能发送 `author_action`；切换到其他作品后，源作品 confirmation 不能在目标作品可见或可执行，且不能派发工具或产生 pending draft；确认等待期间 Work facts 变化时，确认后的 binding / trace 必须消费确认时最新 Work snapshot；确认后的工具失败必须以 failed TurnResult 和作者可见失败文案呈现，且不能生成 pending draft 或 production write。
- Boundary: 已修改过 `novel_domain` behavior snapshot / ConfirmationBinding contract、`novel_application` action validation / cancel TurnResult / confirmation re-gate proof、`novel_web` Channel contract tests 与当前-turn stale guard；`novel_persistence` 只做 TurnResult Ecto mirror 与 JSON SSOT 同步的验证卫生修复；本次只在 `novel_agent` test/support provider 增加确认后工具失败 fixture，不注册进 production runtime；前端只改 `frontend/slice-verify` 外部 driver / verifier / tests，不修改 production UI 自动化逻辑或验收 hook。
...
| novel_agent | yes | 仅 test/support `slice_verify` provider 增加 `AU04FAILTOOL` 确认后 creative tool failure fixture；不注册进 production runtime |
...
| quality | yes | 新增 `quality/acceptance/scenarios/au04-confirm-before-execute.yml`、`quality/acceptance/scenarios/au04-confirm-idempotency-ui.yml`、`quality/acceptance/scenarios/au04-stale-confirmation-ui.yml`、`quality/acceptance/scenarios/au04-confirmation-ttl-ui.yml`、`quality/acceptance/scenarios/au04-disabled-confirmation-action-ui.yml`、`quality/acceptance/scenarios/au04-history-confirmation-readonly.yml`、`quality/acceptance/scenarios/au04-cross-work-confirmation-guard.yml`、`quality/acceptance/scenarios/au04-latest-context-rebase-confirmation.yml`、`quality/acceptance/scenarios/au04-confirmation-tool-failure-recovery.yml` 并登记总表 |
...
| T15 | 确认后工具失败恢复真实 UI 验收 | done | `au04-confirmation-tool-failure-recovery`：真实工作台高风险重写先确认，点击确认后 `prose_writing` provider failure 返回 failed TurnResult，页面显示失败且 no-pending-draft/no-production-write |
| T16 | 完整外部 UI action matrix 验收 | doing | `au10-workbench-recovery-cancel-waiting` 已覆盖取消等待主路径；`au06-single-active-confirmation` 已覆盖同一 workstream 旧 confirmation 不可执行且最新 confirmation 可执行一次；仍需覆盖采纳确认 terminal history 的真实 UI 细节、持久 ConfirmationBinding snapshot、replay、LLM timeout/retry action |
| T17 | 确认卡说明内容真实 UI 验收 | done | `TurnResultBuilder` 从 open confirmation `BehaviorState` 输出 `confirmation_card`；`au04-confirm-before-execute` 现在断言真实页面可见确认对象、确认前 no-tool/no-write 和确认后 re-gate 说明 |
...
- [x] 外部自动化驱动真实页面的场景化验收：`bash scripts/tauri_slice_verify.sh au04-confirmation-tool-failure-recovery`
- [x] 外部自动化驱动真实页面的场景化验收：`bash scripts/tauri_slice_verify.sh au06-single-active-confirmation`
...
- [x] 质量验收入口：`bash scripts/quality_accept.sh au04-confirmation-tool-failure-recovery --surface tauri`
- [x] 质量验收入口：`bash scripts/quality_accept.sh au06-single-active-confirmation --surface tauri`
- [x] 后端 / Channel / 组件局部验证
...
- [x] provider fixture 局部验证
  - `mix test apps/novel_agent/test/novel_agent/provider/slice_verify_test.exs`
- [x] 外部 verifier 局部验证
...
- [x] `bash scripts/task_done.sh --slice au04-confirmation-tool-failure-recovery --skip-static-scan`
...
- 2026-06-20 — 补 AU-04 confirmation tool failure recovery 真实页面验收：`slice_verify` test/support provider 只在 `AU04FAILTOOL` 标记进入 confirmed `prose_writing` creative item 阶段时返回 provider error，生产 runtime 不注册该 provider。`au04-confirmation-tool-failure-recovery` 从真实工作台输入高风险重写、收到确认卡、点击“确认执行”，证明 failed TurnResult 与失败文案可见，`provider_gateway.complete.error` / `toolbox.execute.error` 有日志证据，且无成功 toolbox dispatch、无 pending `prose_fragment`、无 production write。这关闭 E2/GAP-09 的工具失败主路径，不替代 LLM timeout、retry action、持久 snapshot 或 replay。
- 2026-06-21 — 补 AU-04 确认卡说明内容真实页面验收：`TurnResultBuilder` 为 open confirmation behavior 输出 `confirmation_card`，包含 prompt question、确认对象、确认前 no-tool/no-write 和确认后重新 gate 说明；`au04-confirm-before-execute` 外部 driver 必须在真实页面看到这些文字后才点击确认，native verifier 新增 `confirmation_card_explains_target_no_write_and_re_gate_in_real_workbench` 断言。这关闭 B1/GAP-02 的 P0 主路径；持久 ConfirmationBinding snapshot、replay、LLM timeout/retry action 和 AU-06 lifecycle 矩阵仍继续。
- 2026-06-21 — 补 AU-06 single-active confirmation 真实页面验收：`au06-single-active-confirmation` 从真实 Tauri 工作台连续输入两个高风险重写请求；summary 证明第一条 confirmation `behavior_ref=bh_10`、第二条 confirmation `behavior_ref=bh_21`，旧 confirmation 被 hidden/disabled/stale 防住且 no-tool/no-draft，最新 confirmation 仍执行一次并生成单一 pending `prose_fragment`。该 checkpoint 关闭 AU-06 C3 的 live runtime safety，不替代持久 BehaviorBinding ledger 或 replay。
