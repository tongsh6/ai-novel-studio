...
- 状态：doing（AU-04 高风险确认主链与重复确认幂等 checkpoint closed；ConfirmationBinding re-gate refs 局部闭环；完整 action matrix 继续）
...
- Invariant: UI 只能提交服务端 action；高风险执行确认前不得 tool dispatch 或 production write；确认/取消必须绑定 open behavior；确认后必须重新 gate，并留下 `rebased_state_snapshot_ref` / `gate_result_refs` proof；取消/拒绝必须关闭具体 behavior 且 no-write；同一 `idempotency_key` 不重复执行。
- Boundary: 已修改过 `novel_domain` behavior snapshot / ConfirmationBinding contract、`novel_application` action validation / cancel TurnResult / confirmation re-gate proof、`novel_web` Channel contract tests；`novel_persistence` 只做 TurnResult Ecto mirror 与 JSON SSOT 同步的验证卫生修复；本次不修改 provider、production UI 自动化逻辑或验收 hook。
...
- Proof: `mix test apps/novel_domain/test/novel_domain/confirmation_binding_test.exs apps/novel_application/test/novel_application/execution_authority_test.exs apps/novel_application/test/novel_application/action_roundtrip_test.exs apps/novel_web/test/novel_web/channels/workspace_channel_v3_test.exs apps/novel_web/test/novel_web/channels/workspace_channel_action_idempotency_test.exs`、`pnpm --dir frontend test -- --run frontend/slice-verify/native-tauri-verifier.test.mjs`、`bash scripts/tauri_slice_verify.sh au04-confirm-before-execute`、`bash scripts/tauri_slice_verify.sh au04-confirm-idempotency-ui`、`bash scripts/quality_accept.sh au04-confirm-before-execute --surface tauri`、`bash scripts/quality_accept.sh au04-confirm-idempotency-ui --surface tauri`。
...
| quality | yes | 新增 `quality/acceptance/scenarios/au04-confirm-before-execute.yml`、`quality/acceptance/scenarios/au04-confirm-idempotency-ui.yml` 并登记总表 |
...
| T9 | 完整外部 UI action matrix 验收 | todo | `au10-workbench-recovery-cancel-waiting` 已覆盖取消等待主路径；仍需覆盖 disabled/stale、TTL、采纳确认 terminal history 的真实 UI 细节和跨作品/历史会话 |
