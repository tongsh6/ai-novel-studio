...
- 状态：checkpoint closed（high-risk confirmation）；AU05-GAP-06 stale/conflict/cross-work/canon checkpoint 已由后续 AU05 slices 闭环
...
- Acceptance Driver: `bash scripts/tauri_slice_verify.sh au05-adoption-safety-freshness`，外部 Playwright 通过真实 Tauri 工作台输入高风险采纳场景、点击可见“设为后续方向”，并收集 app JSONL、WebSocket frames、截图和 summary。产品代码未新增 slice id、autorun、隐藏 DOM hook 或验收专用 Channel/API。
...
- 高风险候选采纳返回 `require_confirmation` / `needs_confirmation`，UI 显示后续方向待确认 / 确认回合；`truthfulness.candidate_adopted=false`，`production_write_performed=false`。
...
- stale restored candidate rejection、cross-work recovery 与 canon conflict recovery 已由后续 checkpoint 闭环。
- 剩余缺口集中在完整 context version / revision snapshot / canon store 自动计算与 StateTrace 持久化。
