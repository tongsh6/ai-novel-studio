...
- Acceptance Driver: 计划新增 `bash scripts/tauri_slice_verify.sh au02-candidate-adoption-bridge`，外部 Playwright 打开真实 Tauri 工作台，通过可见输入、候选按钮、采纳动作完成验证；产品代码不得新增 slice id、autorun、隐藏 DOM hook 或验收专用 Channel/API。
...
| 候选继续探索 | 已有真实工作台候选点击证据，但不覆盖明确采纳 |
| AdoptionBoundary | application/domain 局部测试存在 |
| 采纳 UI / Channel | AU-05 草稿采纳已有 Tauri 证据，可复用边界经验 |
| 关键缺口 | AU-02 候选卡到 AU-05 adoption boundary 的 author action 链路未闭环 |
