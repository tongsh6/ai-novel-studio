...
| Ordinary conversation unified AgentRun | 新验收口径；`agent-conversation-turn` scenario | `conversation_turn_v1`、`agent-conversation-turn`、ack active run UI、`artifacts/slice-verify/agent-conversation-turn-tauri/summary.json` | 已实现且有证据 | 已从 fixed steps workflow 纯化为 observation-led next-step planner loop。真实 Tauri 已证明 context / frame / strategy / finalize 四个执行 step 在 AgentRun lifecycle 内可见，completion decision 由 planner 裁决完成，`steps=4`、`tool_calls=0`、`provider_calls=6`、planner/conversation provider activity 可见，无 `sync_turn` fallback。 |
...
| T12 | AgentRun runtime planner 纯化 | partial | 已去除外部 `steps:` 启动参数和隐式 `planner || steps` fallback；`character_design_with_context_v1`、`conversation_turn_v1`、正文草稿、章节大纲、角色演化均已接入 provider-backed `AgentNextStepDecision`；AgentRun profile/flow 生产代码已清零 `spec[:complete_fn]` / `spec[:quality_complete_fn]` / `input[:quality_complete_fn]` 旧输入 key。剩余 fixed-step profile 与更底层 public API / legacy caller 兼容登记为后续纯化项。 |
...
- 2026-06-30 — AgentRun runtime planner 入口已纯化：`AgentRunService.start_bounded/start_durable` 不再接受 `steps:`，Channel 只传 `next_step_planner`，`DialoguePlanningService` 去掉 `agent_next_step_planner(...) || AgentRunSequentialPlanner.from_steps(...)` 隐式 fallback。新增 `AgentNextStepDecision` contract；`character_design_with_context_v1` 用 provider-backed next-step planner 根据 observations 决定读阵容、设计角色或完成。剩余同类遗留是显式 fixed-step planner 适配的未迁移 profile，以及 provider `complete_fn` 旧输入形态。
- 2026-06-30 — AgentRun profile provider 输入形态继续纯化：生产 flow 不再读取 `spec[:complete_fn]`、`spec[:quality_complete_fn]` 或 `input[:quality_complete_fn]`，测试侧用 `%NovelAgent.Provider.Execution{}` 显式表达 provider execution dependency；真实 Tauri `agent-bounded-roster-to-character-design` 复跑通过。剩余同类遗留收窄为 fixed-step profile 迁移和更底层 public API / legacy caller 兼容清理。
