...
二轮复核范围只处理第一轮已登记的剩余 P1/P2、external blocker 或 cross-reference，不回退 AU-08 的文件级可交付结论。本轮发现并关闭 1 个 AU-08 cross-reference driver 漂移：`p1-chapter-draft-generation` 旧等待条件仍依赖“待确认的创作材料”文案，当前真实 UI 已展示“待保存章节草稿/章节正文草稿”。修复仅在外部 driver，生产 `frontend/src` 与 umbrella runtime 未改。
...
| SC-AU08-B4 返回工作台不丢上下文 | 已实现未验收 | 缺“阅读返回后继续输入/同 work/session”专项真实页面断言 | 验收缺口 | P1 | 当前多个 reading driver 可进入阅读；尚无 return-context driver | 补返回工作台后继续输入、work/session 不丢的 Tauri driver | 否，本轮不扩产品面 | `au08-reading-return-context` | 是，已登记后续 |
...
| SC-AU08-D1 阅读模式不能编辑或采纳 | 已实现未验收 | 核心 reading drivers 无写入，但缺“阅读模式内无 author_action/write”专项断言；refresh 后续仍待专用化 | 验收缺口 | P1 | `p1-chapter-draft-generation` 二轮证明未采纳草稿阅读反证中 no adopt；`au02-unadopted-candidate-no-reading-fact` 证明 no action/projection/write | 补阅读模式内 no-author-action/no-write 专项 driver，可与 refresh no-write 合并 | 否 | `au08-reading-readonly-no-write` | 是，已登记后续 |
...
| SC-AU08-D4 阅读模式有自动化验收 | 已验收 | 二轮发现单个 cross driver 文案等待漂移；已修复并复跑 | quality harness drift | P1 | 本轮通过 `p1-chapter-adoption-reading`、`p1-chapter-edit-then-accept`、`p1-word-count-audit`、`p1-chapter-expansion-multichapter`、`p1-export-minimum`、`p1-chapter-draft-generation`、`au02-unadopted-candidate-no-reading-fact`、`su02-artifact-projection-trace-isolation` | 已补 `p1-chapter-draft-generation` 外部 driver；后续保持 nightly | 是，已关闭 | AU-08 quality regression | 是 |

二轮退出判断：AU-08 当前没有未关闭的本文件内 P0；唯一应在本轮关闭的 P1/cross-reference driver 漂移已关闭并复跑通过。剩余 P1 都需要新的 projection refresh/status 产品能力或专项异常 driver，不应以当前核心阅读链路证据冒充完整状态机闭环。满足进入 AU-09 的二轮退出标准。
