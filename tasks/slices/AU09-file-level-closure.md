...
- Boundary: 本 checkpoint 只改外部验收 driver/verifier、quality manifest 和 docs/tasks 索引。生产 `frontend/src`、umbrella runtime、provider/runtime 边界、scenario invariant 脚本不改；不新增验收 env/query/localStorage/DOM hook；不把 fixture provider 注册进 production runtime。
...
- Proof: 10 个当前可复跑 Tauri driver；`bash scripts/quality_manifest_check.sh`；`bash scripts/task_done.sh --skip-static-scan && node scripts/task_done_check.mjs`；`bash scripts/ai_static_scan.sh --top 10`。本 checkpoint 不触及 TurnResult/tool/artifact/adoption 主链代码，因此不新增 I1/I2/I3 要求；若后续改主链代码再补跑。
- Acceptance Driver: `au09-memory-create-recall`、`au09-memory-management-entry`、`au09-memory-management-filter-matrix`、`au09-memory-trace-roundtrip`、`au09-adopt-setting-recall`、`au09-character-dossier-roundtrip`、`au09-validity-window-recall`、`au09-cross-work-memory-isolation`、`au09-au03-session-memory-layering`。产品代码新增验收感知逻辑：no。
...
| frontend/slice-verify | yes | 新增 `au09-memory-management-filter-matrix` 外部 driver/verifier；不改 production UI |
...
| # | 任务 | Status | 备注 |
|---|---|---|---|
| T1 | 审计 AU-09 验收文件、蓝图、slice、Tauri list 和 artifacts | done | 当前可复跑入口为 10 个 `au09-*`；`au09-archive-real-data` / `au09-memory-recall-context` 仅作为历史背景 |
| T2 | 新增 AU-09 quality manifests 并登记索引 | done | 10 个当前 Tauri 入口已挂入 `quality_accept` |
| T3 | 重写 AU-09 文件级场景矩阵与 P0/P1/P2 缺口分级 | done | AU-09 当前 12/14 已验收、2/14 部分实现，P0=0 |
| T4 | 同步 SCENARIO-BLUEPRINT、acceptance README、ledger 和 slice 索引 | done | 当前按用户指定顺序可进入 AU-10 |
| T5 | 复跑真实 Tauri / quality acceptance 与质量门禁 | done | 10 个 AU-09 quality acceptance 已通过；task_done/static scan 见 §7 |

## 5. 当前退出判断

- P0：已关闭。当前 10 个真实 Tauri / quality 入口覆盖核心记忆创建/确认/召回、档案概览 stats/detail current driver、管理页可见筛选矩阵、生命周期终态、author-safe trace、伏笔/规则 adoption、角色主档案、有效期窗口、跨作品隔离和 AU-03 会话/记忆分层。
- P1：pending inbox、developer replay、多类型 replay UI、Channel 管理入口、完整独立 MemoryTrace/StateTrace 表、角色 CP2 字段/关系/演化层。普通旧 turn scoped query 与 partial replay UI 已由 AU-07 回填；完整 archive stats current driver 已由 `au09-archive-stats-current` 关闭。
- P2：Phase 0 记忆管理 UI 的正式 Pencil 追溯、排序/分页/limit 控件真实页面验收、scene-level window / ranking 深化、更多空态/错误态组合。

## 6. 二轮剩余缺口矩阵（2026-06-22）

本轮不推翻 2026-06-21 文件级可交付结论，只复核已登记的 P1/P2、external blocker 和 cross-reference。二轮复跑的当前证据为 10 个 AU-09 quality/Tauri 入口：`au09-memory-create-recall`、`au09-archive-stats-current`、`au09-memory-management-entry`、`au09-memory-management-filter-matrix`、`au09-memory-trace-roundtrip`、`au09-adopt-setting-recall`、`au09-character-dossier-roundtrip`、`au09-validity-window-recall`、`au09-cross-work-memory-isolation`、`au09-au03-session-memory-layering`，均通过。AU-09 当前没有 external platform blocker。

| 场景 ID / 名称 | 第一轮状态 | 剩余缺口描述 | 缺口类型 | 优先级 | 当前证据 | 需要补的实现或验收 driver | 是否应在 AU-09 本轮关闭 | 建议 checkpoint / slice | 是否满足继续到 AU-10 的二轮退出标准 |
|---|---|---|---|---|---|---|---|---|---|
| SC-AU09-A1 打开档案看到真实作品全貌 | 部分实现 | 无；完整 stats/detail current driver 已挂回当前 quality/Tauri entrypoint | 验收缺口已关闭 | done | `au09-archive-stats-current-tauri` summary 记录 `archive_volumes=1`、`archive_chapters=1`、`archive_memory_items=3`、`archive_drafts_total=2`、`archive_drafts_accepted=1`、`archive_foreign_excluded=true` | 保持 quality regression | 已关闭 | `AU09-archive-stats-current` | 是，A1 已关闭 |
...
| SC-AU09-B2 筛选、搜索、排序当前作品设定 | 已验收 | 可见 keyword/type/scope/status/locked/组合筛选矩阵已由真实页面关闭；排序/分页/limit UI 控件仍未暴露 | 产品深化 | P2 | `au09-memory-management-filter-matrix` 证明真实工作台创建 3 条当前作品记忆，并通过可见过滤控件隔离匹配行；请求 URL 携带可见 filter 参数 | 后续如产品增加排序/分页/limit 控件，再补 `AU09-memory-management-sort-pagination-ui` | 否，P2 后续 | `AU09-memory-management-sort-pagination-ui` | 是，B2 P1 已关闭 |
...
| SC-AU09-D2 记忆引用可溯源 | 部分实现 | author-safe lifecycle/reference trace 已闭环；普通旧 turn scoped query 与 partial 作者提示已由 AU-07 回填；developer replay、多类型 UI、完整独立 MemoryTrace/StateTrace 聚合未闭环 | Trace/replay 缺口；cross-owner | P1 | `au09-memory-trace-roundtrip`；why 来源来自 `au09-memory-create-recall` / `au09-adopt-setting-recall`；`au07-persisted-trace-query`；`au07-partial-replay-ui`；AU-07/E2E 已覆盖部分 replay cross evidence | AU-07 developer/multi-type replay + AU-09 trace 聚合 follow-up | 否；owner 与 AU-07 共管，当前 author-safe 主链不阻塞 | `AU07-developer-multi-type-replay` + `AU09-memory-state-trace-aggregate` | 是，P1 已登记且不误写为完整 trace |
...
- [x] `bash scripts/quality_accept.sh au09-archive-stats-current --surface tauri`
- [x] `bash scripts/quality_accept.sh au09-memory-management-entry --surface tauri`
- [x] `bash scripts/quality_accept.sh au09-memory-management-filter-matrix --surface tauri`
...
- 2026-06-22 — 二轮缺口收敛不回退文件级可交付结论。`au09-memory-management-filter-matrix` 新增真实 Tauri 证据，关闭 SC-AU09-B2 可见筛选矩阵 P1；`au09-archive-stats-current` 新增真实 Tauri 证据，关闭 SC-AU09-A1 完整 stats/detail current driver P1；AU-09 当前为 12/14 已验收、2/14 部分实现。SC-AU09-A3/D2 继续按 P1 或 cross-owner 登记，B2 排序/分页/limit UI 深化降为 P2，当前无 AU-09 本轮必须关闭的 P0/P1，可进入 AU-10。
