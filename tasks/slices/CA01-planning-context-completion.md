...
- **Consumer**：第一=plot_outline 工具上下文（roster+摘要窗——凭空造角的直接杠杆在工具层非 planner 层，实现中修正认知）；第二=判断 call1 作品上下文块（创作锚自动投影，nil 过滤诚实缺席）；第三=AgenticPlanDraftPlanner 起草层（planning_facts_section，**本轮余项**）。
...
- **Exploration**（08 §8 探索面同步律，2026-07-21 升格第七问，本 slice 即首个按七问口径收口者）：创作锚三字段同批入 `archive_read` profile 档案面渲染（前提/主题/主线目标三行，缺席诚实不渲染）✓——该缺口由用户"七问"追问抓出后当场补齐；卷投影 ⓒ 探索可达随其落地时同批处理（余项）。
